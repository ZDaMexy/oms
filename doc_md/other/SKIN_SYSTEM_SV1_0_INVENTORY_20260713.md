...
| 证据 | 判读 |
| --- | --- |
| length / mtime / SHA-256 | 取证前后逐项一致；精确值只保存在仓库外脱敏恢复归档，不写入仓库 |
| OMS/osu 进程 | 取证前后均为 `0`，无客户端写入源 |
...
| 迁移证据 | 结果 |
| --- | --- |
| 迁移前备份 | 已完成 SHA-256 核验；精确值只保存在仓库外脱敏恢复归档 |
| 生产迁移后 | SHA-256 相对迁移前发生变化，且独立只读重开后保持稳定；精确值不写入仓库 |
| Realm length | 迁移前后保持一致 |
| `SkinInfo` | `3 → 2` |
