...
5. G1 使用先前已验证的**真实备份 baseline 的一次性工作副本**；路径等私有续接值见本机 `%TEMP%/oms-c7-resume-private.json`。该 baseline25文件/175303768字节，原有0非protected皮肤记录、18未知blob；新 G1 在一次性工作副本注入真正预存普通皮肤验证非空保护。不要把它写成原根本来有非空用户记录，不打开用户原 Realm。重跑前后只读摘要核对。
