...
10. **`BMS -> mania` 转谱 BGM 的 mania-runtime 呈现保真与性能归本子线 J6**（与 P1-K K11 的"转谱对象语义"分线；K11 + J6 首版均 2026-06-01 落地，详见 CHANGELOG）：J6 首版采用**复用 `BmsKeysoundStore`** 而非「先试对象池、不达标再共享池」——因 **E（暂停停 BGM）必须有中心化 pause-aware store 才能解**（一次性样本不随暂停停止），对象池只能帮 D；故转谱 BGM/scratch 直接经该 store 播放（pause/seek 统一停 + 通道上限）。**注意：E 已 2026-06-01 实测修复，但 D（dense 极端谱高密段）仍极度缓慢——共享 store 已排除「音频对象数」为主因，真瓶颈（疑 drawable 数量 / 转换链 / 渲染）待 profile，D 后置。** 下文要求即由此实现：转谱补出的 BGM sample-only 对象必须在 mania 游玩时 autoplay 出声，使纯键音 BMS 的 mania 转谱音频与 BMS 原生模式一致；不得因 mania 无 `BmsKeysoundStore` 就让 BGM 静音。其 dense-BGM 播放期性能（数千 BGM 事件 → 每对象 `SkinnableSound` 的 alloc/GC/首帧懒初始化）属本子线 dense-chart 热路径治理：首选复用 mania 对象池 / 滚动窗口（`HitObjectContainer` 只激活可见区间对象）；仅当实测不达标时才允许引入 mania 侧共享样本通道池（复用 `BmsKeysoundStore` 的 idle-first / per-WAV cut 思路、按 `BmsBgmEvent.KeysoundId` 归组），**不得为此新长出 per-note / per-lane 独立 sample player（沿用第 1 条）**。同时，mania 转谱的 LN 尾必须与本节第 3a 条一致保持静音：转谱器（K11）不得把尾 keysound 写进 mania `HoldNote.NodeSamples[1]`，否则 mania `TailNote` 会在 release 播放它并对 LNTYPE1 尾复用头 WAV 的谱复现 double。**（2026-06-06）让转谱 note/LN 也走该共享 store 补 per-WAV cut 的尝试已失败、回退到 J6 v1；目标仍成立、但须按下列约束重做。** 目标：转谱 note/LN head 的 keysound 应经 store 获得 per-WAV cut（否则同一 WAV 槽在 store(BGM/scratch) 与 mania 采样 两路径之间、或同槽连续触发之间无法互相掐断 → BMS 原生没有的重复；实测 `macchitodoncho` hyper：同槽近距重触发数百次、BGM↔KEY 跨路径共享槽 150 个）。本次两处实现均回归：① LN 子类 + **非池化自定义嵌套 head** → `DrawableHoldNote.Update()` 无条件访问 `Head`/`headContainer.Child` 时容器空 → `Cannot call InternalChild ... (currently 0)` 崩溃；② tap note 走 store 后某 punai 吉他切片**完全静音**（0 次播放、非单声部）——通道饱和已被数据排除（BGM-only 峰值 27–36 已近 32 上限、加 KEY 仅到 32–41）、单对同槽 cut 逻辑上应得 1 次而非 0 次，**真因未定性**。**再尝试约束**：(a) 不得用非池化自定义嵌套 hold drawable（必崩；嵌套 head 须保持池化类型，或改用不依赖自定义嵌套 drawable 的接法）；(b) 不得靠静态推理在生产里试 store 路由——须先有 **player-level 集成测试 + 运行时日志**定性静音真因（**2026-06-07 已满足并实测**：`BmsKeysoundStore` opt-in 零开销播放日志 `KeysoundPlaybackRecord` + `TestSceneBmsToManiaKeysoundPlayback` + 实验性 gated tap-note 路由 `BmsConvertedKeyNoteHitObject`/`RouteKeyNotesThroughStoreForTesting` + `TestSceneBmsToManiaKeyNoteStoreRouting`；**实测 tap-note 路由本身无静音**——每个命中 tap 音符都进 store，2026-06-06「0 次播放」未复现、系 harness/生产观测假象或 LN 旁路，gated 脚手架默认 off 保留作转正根基；headless 未覆盖 per-WAV-cut 复用路径，见 CHANGELOG/STATUS）；(c) 仍不得为挽回池化而新长出 per-note / per-lane 独立 sample player（沿用第 1 条）。在此之前，转谱 note/LN keysound 维持 mania 一次性播放（转谱键音重复=已知遗留）。**（2026-06-07 更新）tap-note 部分已转正**：harness 实测 tap-note→store 路由无静音（旧「0 次播放」未复现、系 harness 假象/LN 旁路）后，按用户决定把 tap-note 路由转为生产默认（`BmsConvertedKeyNoteHitObject`+`DrawableBmsConvertedKeyNote`，`ShouldHost` 含 KEY 音符）；per-WAV cut 现跨 BGM↔KEY+KEY↔KEY(tap) 统一生效。**遗留约束**：① **LN head/tail 仍维持 mania 一次性**（LN 走 store 须池化嵌套头、未做）；② 🔴 **非池化 perf**——`Note` 子类必非池化（mania 精确类型池化），每个转谱 tap 音符为非池化 drawable、对 dense(D) 可能加压，pooling-preserving 替代（mania core 让池化 `DrawableNote` 走 shared store、不碰第 1 条）= perf 回归时正解；③ 待用户真实谱实测 per-WAV-cut 复用路径（headless 测不到）与 dense perf，出问题则回退。**（2026-06-07）全链路审查后落地两条不动对象模型的确定缺陷修复：(i) mania 转谱 keysound prewarm——`DrawableManiaRuleset.LoadComplete()` 在 converted-BMS + `ModAutoplay` 下遍历 `Beatmap.HitObjects` 的 `Samples`/`HoldNote.NodeSamples` 调 `Playfield.PrepareSamplePool`，对齐 BMS 原生 `DrawableBmsRuleset.PrewarmKeysounds`（此前 mania 转谱完全无预热，KEY note keysound 首播即首次磁盘加载）；(ii) 转谱 store 通道 floor 128——`BmsToManiaKeysoundStoreFactory.Create(IRulesetConfigCache?)` 设 `store.ConcurrentChannels = Math.Max(config 的 KeysoundConcurrentChannels, 128)`（mania 反射委托传 `IRulesetConfigCache`、cache 缺席回退默认）。**(ii) 的起因 = 2026-06-07 用户实测确认的真因**：autoplay/游玩下「BGM 人声丢失」是单个长 BGM `bgm1`（measure 1 单次触发、712KB、整首仅 1 次）被 store 32 通道饱和时轮转偷取掐断（该谱 channel-01 共 4032 事件、并发峰值 27–36 > 32，长 BGM 占通道最久被偷），**非** prewarm、**非**惰性 `LoadSamples`、**非** KEY-note 路径（争议人声是 BGM bgm1 长样本）。mania 转谱 store 专播 BGM 自动层 + scratch（必须完整的背景音乐、非玩家 polyphony 预算），floor 128 远超峰值 → 长 BGM 永不被偷；用户调更高 config（≤256）仍生效。性质=治标的务实近似（提高 polyphony 到远超峰值）；治本（长 BGM 不进可偷池 / BGM 与 keysound 分池 / 偷取保护长样本）碰本子线第 8 条 dense hot-path 红线（idle-first 不回退全表扫描）、后置。BMS 原生 store 未动（兼播玩家键音、config 是玩家 polyphony 偏好；用户「BMS 正常」基准疑为 beatoraja）。两者均不得为之新长出 per-note/per-lane 独立 player（沿用第 1 条）；prewarm 必须复用 core `Playfield.PrepareSamplePool`、不得引入第二套 retained sample authority（沿用第 7 条）。per-WAV cut（note/LN 走 store）仍按本条上述「再尝试约束」后置（= 独立遗留，与本次 BGM 丢失无关）。autoplay/游玩 BGM 人声丢失已于 2026-06-07 **确诊并修复**（见上 (ii)，用户实测 autoplay+游玩均恢复 ✅）；此前「prewarm 缓解加载延迟 / 惰性 `LoadSamples` 时序窗口」的强假设已被实测推翻、作废。**
