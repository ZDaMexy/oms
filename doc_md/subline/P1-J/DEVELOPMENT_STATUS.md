...
- 2026-05-17：`BmsKeysoundStore` 现已补上 gameplay pause / seek 生命周期回收：监听 `GameplayClockContainer.IsPaused` 与 `OnSeek`，在边界处统一 `StopAllPlayback()`，避免默认只停 looping sample 的通用语义让长 one-shot BGM / keysound 穿透暂停或拖拽。新增 `TestSceneBmsKeysoundPlaybackLifecycle` 锁住 pause 与 seek 两条链，随后连同 `TestSceneBmsSharedKeysoundTiming` 与 `TestSceneBmsKeysoundChannelConfigBinding` 一并回跑，shared keysound 相关 focused suite **9/9** 通过。
...
- 2026-05-18：针对 dense full autoplay 剩余的“整局只卡一次”继续补窄修补：确认 core `Playfield` 的 sample pool 预热不会自动覆盖 BMS gameplay keysound 后，新增 `Playfield.PrepareSamplePool()`、`BmsPlayfield.PrewarmKeysounds()` 与 `DrawableBmsRuleset.LoadComplete()` 侧的 unique keysound prewarm，把 full autoplay 首次命中的 sample pool 初始化前移到进场加载。focused validation：`dotnet test osu.Game.Rulesets.Bms.Tests --no-restore -v minimal --filter "FullyQualifiedName~TestSceneBmsAutoplayReplayPlayback|FullyQualifiedName~TestAutoPlayObjectsStillApplyMaxResult"` **4/4** 通过；`dotnet test osu.Game.Rulesets.Bms.Tests --no-restore -v minimal --filter "FullyQualifiedName~TestSceneBmsSharedKeysoundTiming|FullyQualifiedName~TestSceneBmsKeysoundPlaybackLifecycle|FullyQualifiedName~TestSceneBmsKeysoundChannelConfigBinding"` **9/9** 通过。
