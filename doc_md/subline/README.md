...
| 子线 | 负责范围 | 当前判定 | 下一道门 |
| --- | --- | --- | --- |
| [P1-A](P1-A/DEVELOPMENT_STATUS.md) | 产品面、Skin V1、release gate | `SV1-0` 全过；managed `.osk` BMS Note/LN head/body/tail 四组件自动 gate 闭合，`V-001`～`V-004` 签收 0/4；`SV1-2` authority/path preflight 与 pure capsule 已落但均无生产调用 | Windows native handle-relative/no-follow capture → production folder factory/选择 → scanner/mutation ownership → 整包原子重载；`SV1-1`/Skin V1 整体仍未完成 |
