...
- 测试只访问deterministic fake及系统临时目录；junction定点创建/清理，生产Realm、`chartskin/`、用户目录、网络与GUI零访问。当前全仓除tests外无`SkinManager`/production managed folder factory/选择消费方；未接`InstantiationInfo`/选择、external source、scanner owner、mutation或atomic reload，产品可见变化为0，`SV1-2`、G1、Skin V1与视觉签收状态均未完成/未改变。

### `SV1-2` 第二刀：ruleset-neutral immutable package revision capsule 合同

- shared core 新增纯 post-capture `SkinPackageRevisionCapsule`：输入是 capture producer 提供的稳定逻辑 file/directory entries，不含 filesystem path、Storage 或 authority；本节当时没有 producer，现已有第三刀 managed native internal producer，但仍无产品消费方。它不是 directory inventory/capture service，不证明 containment、no-follow、physical identity、TOCTOU 或读取期间稳定性。
