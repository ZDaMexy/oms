...
- 用户实际体验后明确否定星轨的动画、美术安排和精细度，认为当前不可用；已将“只待观察”修正为有实际待改内容。静线继续默认与保底，但没有被正式签收；此前“暂时没问题”只针对启动/预览及告警修复反馈，不扩大到两款作品或整套系统。
- 用户要求本轮只同步文档、记忆、commit 和 push，后续新对话再优化 simple/complex；不改程序、作者源、素材、工具或已校验发行包，不生成新设计决定或新阶段。既有 7/7 历史工程记录及自动证据保留，不用它宣称产品验收完成或没有作品修改。
- 总体否定不虚构具体设备、键数、样式或矩阵单项失败；V-001～V-004 仍 0/4、V-005 未签，设备/长期门和旧 OmsSkin 删除实机门保留。文档检查、独立复核与差异检查用于本轮收尾；未改运行输入，不重跑游戏、Release、安装或作者演练，之前运行日期和证据不刷新。详见 [暂停记录](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md#2026-09-12-用户体验结论与本轮暂停)。

### 原 C7 默认皮肤提示、普通导入说明与构建警告修复

- 依据用户实际 Release 运行日志，为 BMS 5K/7K 的左右转盘声明补普通作者可用的 presentation 选择条件，并兼容普通导入在公共段末尾添加的整行说明。当前样式的坐标错误、行内错误、BOM、资源准入及来源/权限保护保持严格，真实诊断保留完整日志和中文操作指引。静线仍为首次默认与正式保底；星轨继续是展示包和默认候选，不擅自改变首次选择。
- MessagePack/SignalR 更新为已确认的同支修补版本，移除历史全局告警屏蔽；用户实际使用的 VS Code Release 同命令构建零警告，不把此前 Debug 检查当作真实运行配置。未启用联网，也未把仍存在的具名例外和 SQLite 依赖风险称为已清零。根因、边界、独立源码复核及风险依据见 [C7 当前修复](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md#交付后默认皮肤提示与构建警告修复)。
- 当前完整 BMS 复验 2220/2220；mania 863/867、core `~Skin` 1339/1344、FileStore 11/11，原四项/五项失败按名称、类别和完整消息逐条相同；共享定点 104/104、真实两玩法产品与入口 21/21 通过。首轮 BMS 五项选择失败、同产物精确定点六项通过和之后完整通过全部保留，未确定首败原因，不追认旧基线、不放宽断言。原 021 红测、008 失败矩阵与主动中止也不计为通过。
- 新作者工具完成独立制作演练并同步 WORKSHOP 与两份记录；`oms_20260911_startup-fix-final.zip` 和 `oms-skin-c7-acceptance-20260911-startup-fix-final/` 已实际生成。四轮安装恢复、两测试根玩法可用之外，另从 preview-fix 隔离安装完成真实跨版本更新，由游戏首次启动自动更新默认工作副本，并从第三份新复制的只读测试数据库确认两玩法可用。原三处保存根字节/属性一致、原库未打开；精确制品身份与过程证据见上方报告。本轮最终独立产物复核通过。
- 原七阶段不重计。旧预览入口事故、原失败及误入保存根事故历史保留，后者没有事前快照，不能用本轮根未变追溯称无损。V-001～V-004 仍 0/4，V-005 和 C7 观感/设备/长时体验未签；旧 OmsSkin 物理删除仍待实机门，Skin V1 与 release 整体未完成，推送仍须另行同意。

## 2026-09-11

### C7 交付后 BMS 无法进入预览

- 用户日志两次在真实 PlayerLoader 进入时报告 BMS 最终玩法配置缺失。BMS 布局准备改为从正式 RulesetConfigCache 获取与 renderer 相同的配置，保持缺失时明确失败；没有引入默认值或放松布局、权限与 live reload 保护。
- 新增真实 OsuGame → PlayerLoader 的普通/自动演示入口回归，父容器不得提前注入具体配置，使用非默认 P2/Up 核同一布局、重试、退出和重新载入拒绝。修复前 BMS 两入口精确重现用户异常，mania 对照正常；旧宿主配置遮蔽及样式矩阵纠错、修复后规定检查和替代交付证据见 [C7 入口修复](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md#交付后-bms-预览入口修复)。原七阶段不重计，旧完成声明不能代替这次缺陷的实际修复，V-001～V-005 和 C7 人工未签事实保持。
- 修复后定点 142/142、BMS full 2220/2220、规定格式和 Debug/Release 均通过；新作者工具完成无 Git/SDK 的独立复做，两款作品字节未变。修复 ZIP `oms_20260911_preview-fix.zip` 与同名用途集中目录通过真实解压、四轮正常安装恢复/覆盖启动、两测试根玩法可用和实际组装，原保存根前后未变。旧包及证据保留；当前版本与精确 SHA、独立复核和范围见上方 C7 报告，未重签人工或整体发行。

### C7 完整作品、独立制作与使用验证收尾

- 继续原 C7，不重计或拆分七阶段。静线与星轨两款完整普通作者包同时支持 BMS、mania，保留完整作者文件、可重复生成和打包入口、完整公开参考与整体设计预览。静线承担正式保底；星轨提供独立舞台、动画和随游玩变化的组合演出，仍是展示包与默认候选，不锁定首次默认选择。旧 `OmsSkin` 仅保留历史/人工对照，不重新进入产品回退链。
- 已使用真实发布的独立作者工具，在 PS5、PATH 无 Git/SDK 的新套件目录从模板重做 Aurora Study；修改作品资料及明确保留的 README、检查错误、修复、打包、准备导入/更新副本，正式作品及上一版本保留。三款作品检查、重复打包与重新生成后的成品逐字节一致。首次只改作品资料而未改 README 的差异作为真实演练记录保留，不靠覆盖作者文字或修改预期凑一致。制作与游戏普通导入、三来源选择、导出再导入通过同字节成品相连，鼠标和视觉操作不代签，见 [WORKSHOP](../../../skin-authoring/docs/WORKSHOP.md)。
- 当前实际产品复验覆盖完整分数/准确率/连击/进度、窄屏布局、按下反馈、声音、缺件必要回退与可选关闭，以及星轨真实击打、BMS 长条早松、暂停、授权撤销、重试与跳转。真实早松触发的错误事件回退已按同一对象进度来源修正；无当前判定时额外文字不再误报 MISS。后续整套检查暴露的第三方信息重复、历史程序化对照来源及检查宿主缺项分别按真实产品合同修正，保留必要信息实显、舞台隔离、可选关闭和原几何要求。mania 最后的真实输入检查用正常对象池登记动态样本，原异常栈证明旧夹具直接插入显示对象绕过既有身份登记；修正后真实对象、Seek 新 epoch、Perfect 显示和自然退场均通过，未放宽生产身份或时序保护。证据集中于 [C7 报告](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md)。
- 最终当前产物完整复验：BMS **2215/2215**、mania **863/867**、core `~Skin` **1314/1319**、FileStore **11/11**。mania 原四项和 core 原五项失败的名称及完整消息均精确匹配独立旧基线，原 core sample 对照的解决事实另列保留，`closure-failure-comparison.json` 通过；规定格式检查和 Release 编译通过。原始 `c7-closure-*.trx` 与完整比较记录保留在 `artifacts/skin-c7-evidence/`，不以候选包、旧编译或单项通过冒充最终结果。
- 最终随包作者工具独立发布后，已再次完成 PS5、PATH 无 Git/SDK 的完整制作工作流及三包重复制作检查；工具 SHA-256 为 `c478dac99f66fc2cfacd74b3f03d1617047885f24d0b43616047f71cff513aaf`，工作流时间 `2026-09-11T08:37:54.7568409Z`，工具检查时间 `2026-09-11T08:38:06.9279899Z`。两份新证据逐字节同步进作者套件，旧 WORKSHOP 和两份记录先原样保全；实际工具先独立演练后供最终 ZIP 使用，没有虚构从尚未生成的新 ZIP 抽出执行。

### 发行保存位置纠错、最终交付与原 C7 非人工收口

- 旧完整自解压候选实际改变程序基准目录，未读到安装旁便携标记，误入当前账户既有自定义保存根并运行 Realm schema 57 迁移。已完整逐字节保全事后数据与指针，但没有该根事前快照；不能宣称无损、回滚或未知事前后一致性，公开记录不披露账户路径。原窗口/进程 smoke 历史保留，但不能作为保存根正确的证据，详见 [P1-F 历史](../P1-F/CHANGELOG.md)。
- 游戏发行已改为完整 self-contained 多文件 ZIP，并将便携模式传入桌面宿主以令缓存留在程序旁。修正后的真实便携副本已打开预期独立 `data/`、准备正确 canonical 工作副本、完成八秒稳定运行并正常退出，该轮事前保全的三处既有目录在运行后内容和属性一致；该结论仅属于这轮隔离复验，不能倒推前次事故无损。随后真实启动揭示恢复步骤不能进入外层启动互斥的问题，已用专用恢复子凭据修复，保持外层发现与选择重试的同一所有权；退出取消时最终凭据校验前的资源转移泄漏也经真实文件句柄红测复现并修复，只有成功校验才转移所有权。本轮最终自动复验已包含相关恢复及启动路径。
- 最终真实发行物 `release-repo/oms_20260911_4.zip` 已生成，SHA-256 `bcf6aa8da7700f822db6613734dfc4af20d4bf57c5ff6d29f25cf5c4b2ce9ac8`；Windows Shell 解压后的只读 canonical 与摘要检查通过，中文说明及 PS5 编码修正进入最终包，中间候选仅保留为纠错证据。`2026-09-11T09:08:41.8937534Z`～`09:10:58.7608168Z` 的首次便携、自定义保存、工作副本恢复、完整覆盖后启动四轮全部 Passed，均正常退出、ExitCode 0、无强制终止且来源不变。首次便携根与最终覆盖后的自定义根另有真实数据库 BMS/mania `Available=true` 证据；中间两轮不冒称具备独立数据库快照，详见 `release-startup-final4/results.json`、`release-final4-rulesets-portable.log` 与 `release-final4-rulesets-custom.log`。
- 从最终发行物随包入口，在 PS5、PATH 无 Git/SDK 环境实际生成 `release-repo/oms-skin-c7-acceptance-20260911-final/`，退出码 0、来源不变；`acceptance-final4-assembly.json` 保存完整身份和组装证据。验收目录携带两款成品、完整作者工具与源文件、第三方和原有签收输入，可直接执行集中清单；作者工具与此前独立发布演练是同一字节版本。
- 原七阶段工程记录推进为 **`7/7 closed，C7 非人工结果完成`**；最终独立产物复核、完整文档检查、暂存及工作副本差异检查通过，收尾记录与源码在当前分支归档，不重新计数、不拆出新阶段。`V-001`～`V-004` 仍 **0/4**，`V-005` 和 C7 画面、设备、长时间体验未签收；旧 OmsSkin 已退出产品回退链，仅保留历史/人工对照，源码物理删除等待原实机门。星轨仍为展示包和默认候选，SV1-1 整体签收、Skin V1 与 release 仍未完成，推送另行取得用户同意。

## 2026-09-09

### C7 双包、作者套件与安装恢复检查点

- 保全本轮双包及完整公开作者制作路径、canonical 安装接管/恢复、旧数据保护、验收与更新工具。用户因额度要求立即暂停，源码全部冻结；C7 仍 active，不记七阶段或 Skin V1/release 完成，不推送。
- 实际复核修正首次作者目录登记、旧固定 ID 导出、安装原件保留、作者目录元数据上限、击打声音，并发现必要信息缺失、mania 按下图未使用、窄舞台重复文字与测试计分连接缺口。最新源码已保全，但成品尚未重生成，最新测试未编译/执行。
- 已验结果、未验证修改、私有证据定位及恢复执行顺序统一见 [暂停检查点](../../other/SKIN_SYSTEM_C7_RESUME_20260909.md)与 [C7 报告](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md)。原 V-001～004 0/4、V-005 未签收继续保留。

### C6 收尾与产品成果复核

- 按用户要求结束本轮开发，两份独立只读复核确认导入、选择、设置授权、真实 BMS/mania 游玩和 Momentum 候选均有实际入口；本次范围未发现明显重复使用路径或无法到达的新增功能。安全与数据保护有产品价值，但不计新增画面成果，也不按代码量、测试数量或六个已完成阶段估算最终完成百分比。
- STATUS 用玩家和作者可完成的事情说明进度；PLAN 明确下一阶段尽早交付完整双包及独立制作流程。Momentum 仍借用现有基础外观，完整复杂皮肤、完整创作套件和最终默认外观替代未交付；没有可视化编辑器，人工观感和设备验收未签收。C7 的完整退出要求保持不变，本轮未开始实施。
- 同步主线摘要与既有产品进度记忆，默认产品语言及用户结束本轮的边界记录于 [AGENTS](../../../AGENTS.md#工作流)和工作流记忆。本次仅修改文档，不重跑或刷新 C6 产品验证，不改写前次报告中未推送的历史事实；用户已授权本轮提交并推送。
- 文档检查发现 PLAN 混入会话结束说明后，已将该说明限于 STATUS/CHANGELOG，PLAN 只保留成品、制作流程重点与协作规则链接。修正后 `CheckDocumentation.ps1` 通过（143 个 Markdown、1264 个相对链接、127 个本地锚点、105 个 memory 链），`git diff --check` 通过；仅保留既有公开 C4 checksum 提示。
...
- C5在C4 exact package+layout+material publication之上完成唯一versioned scene/event纵切，形成不可拆分的package+layout+material+scene publication：`gameplay-skin.json`与`gameplay-skin.scene.json`分别遵循`oms-gameplay-skin-manifest.v1`、`oms-gameplay-skin-scene.v1`，`GameplaySkinSceneCodec`执行严格UTF-8、duplicate/unknown/type/index/target/resource/path与canonical round-trip校验；资源、模板、graph、纹理、effect、track/keyframe、text/glyph预算在background prepare执行，renderer只实例化单一immutable prepared graph，不重读或二次resolve。
- allowlist覆盖Sprite、Container、Text、Mask、Clip、blend/effect preset、frame/tween/track、state machine、只读property binding、template/instance，并固定layer/z-order、clip/mask、anchor/scale、DPI/safe-area和C3 layout-relative target。任一scene/slot运行异常只熔断该scene/slot并保留判定、layout、material与旧publication；禁止任意shader、script、反射、动态类型、filesystem/network或私有visual effect authority。
- `oms-gameplay-skin-event.v1`由真实BMS/mania/core host生产；envelope固定contract/version、epoch、monotonic sequence、gameplay/layout/material/scene revision、authoritative gameplay time、LaneId/GroupId、kind与immutable payload，覆盖lifecycle、layout/publication、input、object、judgement、score/combo/gauge、timing/beat/bar/BPM与BGA只读摘要。bounded single-producer stream以Snapshot/Reset支持attach/reload/seek/retry/rewind、late attach和epoch切换；engine envelope可发`GameplayResumed`，scene state-machine ABI刻意拒绝`gameplay.resume`，因为完整Snapshot已可重建Running状态。
- `GameplaySkinEventStreamCursor`、scene runtime、timing projection与BMS/mania hosts均进入真实ruleset caller，继承C2 participant/work lease、fresh attach barrier、commit前detach、late attach、跨revision holder、cancel/reentrant/scheduler/dispatcher/shutdown与最后lease exactly-once retire。prepared carrier创建后取消、commit admission失败与日志故障均不得留下provisional资源或拆分publication。
- `GameplaySkinRuntimeSupportProfile`版本化声明每个catalog slot的Supported/NotApplicable/Unsupported决策：BMS 28项均有production route（9K适用矩阵26项，Turntable/Laser按catalog applicability排除）；mania 23项Supported，`object.mine`、`playfield.turntable`、`playfield.laser`、`bga.viewport`、`bga.frame`明确NotApplicable。全部global/stage/group/lane roots、lane surface/divider、judgement/hit target、cover、barline、stage/backdrop/baseplate、key/keyflash/mine、hit explosion、judgement/combo/gauge/text HUD、turntable/laser、BGA frame/viewport与decoration均由固定host或scene route接通；Note/LN/Hold与effect/HUD走有界pool，不逐note解析或无界分配。
- 真实产品矩阵覆盖BMS 5K/7K/9K/14K、14K双deck、Scratch/Special与mania single/dual；Mirror/Random只改变对象最终LaneId，resource/keysound/scene/event跟随该LaneId且topology不变。dense 14K、持续LN、密集judgement/effect/HUD/event压力下，manifest/scene/resource/node/depth/template/effect/track/keyframe/text/event queue/pool预算均在prepare/runtime硬门内；相同输入/revision生成确定事件顺序、动画采样、状态转换、Snapshot/Reset与resource选择。
- 红测从真实ordinary/managed/external source、SkinManager current revision、C4 codec/catalog/resolver/material与BMS/mania producer进入actual renderer；schema/unknown/path/security、graph/animation/state/binding、epoch/Snapshot、late attach/seek/rewind、全slot/capability、mod后LaneId、pool/performance、失败保A与cancel/reentrant/shutdown均覆盖，未使用测试publisher注入最终scene snapshot。foundation审计将event cursor/scene factory/pool标为已接production；通用capability negotiator/authorization、script VM与最终整包reload明确留C6，不计C5。
- 验证：core `~GameplaySkin` **429/429**；mania `~GameplaySkin` **69/69**；BMS `~GameplaySkin` **146/146**；BMS current-revision **215/215**、full（`--blame-hang --blame-hang-timeout 5m`）**1721/1721**且无hang artifact；mania full **860/864**仅冻结既有四项HoldNote frame-count失败；core `~Skin` **1218/1224**仅冻结既有六项失败，`~Skins` **746/750**仅冻结既有四项archive失败；P1-K decoder/converter/timing/keysound **126/126、24/24、17/17、68/68**；Release **0 error / 20 emitted known warnings**。`CheckDocumentation.ps1`、`git diff --check`、owning-project targeted formatter `--verify-no-changes`通过；四类独立终审均GO，blocker/major **0/0**。
- C5因此推进为 **`5/7 closed，C6 active`**。C6仍负责sandbox/script权限、最终ini/manifest/scene/script/素材整包reload gate；C7负责canonical `oms-simple/oms-complex`、Authoring Kit、validator、只读恢复、程序化`OmsSkin`退出与自动release；P1-L继续拥有BGA内容/timeline/seek/gimmick，在线服务、watcher、判定/input/clock/binding authority均不在C5。
...
- BMS与mania真实consumer已迁入同一package+layout+material+scene publication：ordinary Realm`.osk`、managed与registered external三源public声明经真实`SkinManager` current revision、ruleset prepare驱动actual BMS Note/LN与mania Note/Hold/KeyVisual；renderer只读prepared material/scene，不在commit后重开资源或重跑resolver。BMS legacy候选production顺序固定为5K `[Bms]→Keys6→Keys5`、7K `→Keys8→Keys7`、9K `→Keys9`且不重复、14K `→Keys16→同一Keys8 bucket的两个stage-local deck→Keys14`；9K raw`0..8`/canonical`1..9`只走`bms-gameplay-skin-nine-key-index.v1`，Mirror/Random继续只改变mod后LaneId。
- C2+C3+C5 revision协议扩展为exact package+layout+material+scene quadruple：全部codec/catalog/resource/material/scene失败止于background prepare，update thread只交换prepared immutable引用；prepare前后与commit复核participant generation、current selection、exact source/content/package/layout/material/scene及contract version。fresh attach、commit前detach、late attach、跨revision holder、latest-wins/reentrant/cancel/scheduler fault/shutdown与最后lease detach exactly-once retire继续成立，失败保留exact A画面。BMS static与固定60FPS作者合同、P1-K末端lane keysound及C3 geometry/aspect/DPI/safe-area均保持。
