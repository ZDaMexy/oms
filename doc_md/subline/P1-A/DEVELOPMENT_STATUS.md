...
2. 继续为 tri-mode settings 与 pre-start hold overlay 补 visual / input-path integration coverage；当前已锁住 overlay owner contract、提前松开 delayed-start、hold 跨过 delay 仍可调速、persistent target cycle 与 real-player mode/value binding，剩余重点转向更完整 host boundary、fallback 与真实输入事件路径。
