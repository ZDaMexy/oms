...
- **安全回落已覆盖本纵切**：beatmap-local 视觉优先；selected 单槽缺失、损坏、越权或超预算时逐组件回落；跨 package 不拼接资源，异步换源只发布当前 revision 的完整结果。
...
| 4 | managed `.osk` BMS 普通短键编号帧动画实机 | **待用户单独确认**；不可复用静态恢复结论 |
...
- 产品自动验收 **26/26**：真实 `.osk` 导入/游玩对象/Ruleset 链、14K S2、帧推进与循环、SkinManager A→B、beatmap-local 优先、同包坏轨逐组件回落、跨包隔离及异步换源。
...
1. 由用户单独确认 managed `.osk` 的 BMS 普通短键编号帧动画观感与切换/回落表现。
