...
| 皮肤创作生态 `F1`：ini 数据层 → 配置源 → 最小闭环 → ①类铺开 → reference skin | 进行中（①刀解析 + ②刀配置源[机制+接入]已落，978/978 + core gate 绿） | gate 定方案 A。**①刀** ini 解析三件套（不侵入核心）+ **②刀** `BmsLegacySkin`（机制：`ParseConfigurationStream` hook 解析 BMS 段 + `GetConfig` 应答）+ 接入（`SkinImporter` 路由 plain `LegacySkin`→`BmsLegacySkin`，最小 core 改动 + fallback 保护 + 连带 `SkinnableSprite` 精确匹配避 `LegacyBeatmapSkin` bug）已落、17 用例。**③刀进行中**：颜色件 5 件（note/LN head/LN body/lane bg/divider 读 `GetBmsSkinConfig` ?? palette）+ **纹理 note 件全家**（`DefaultBmsNoteDisplayBase` `Box→CompositeDrawable`，贴图优先[Sprite·文件皮肤主导]/ 颜色回退[Box]，body 三态保留），BMS 全套 **986/986**。SKINNING.md 已据代码更正。**待**：剩余纹理（lane/stage/hit target/cover/`KeyImage`）+ 颜色 hit target/barline/cover → 几何（`CreateDefault` 读 config）→ reference skin |
