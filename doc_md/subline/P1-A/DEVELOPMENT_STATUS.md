...
`SV1-0` 自动、schema 56 数据与用户实机 gate 已全部通过；`SV1-1` 的首个 managed `.osk` BMS Note/LN 产品纵切现已覆盖普通短键与长条 head/body/tail，四组件的自动、合同、安全和回退 gate 均已闭合，`V-001`～`V-004` 集中待验收。这只表示首个产品纵切自动闭环，不是 `SV1-1` 完成或产品交付；视觉待签收不再串行阻塞后续自动可证切片。`SV1-2` 第四刀现已让生产 `SkinManager` 对 Realm 中已注册且合法的 managed `chartskin/<direct-child>` 记录完成异步 native no-follow capture → exact allowlisted `BmsLegacySkin` factory → 双重记录复核 → 原子发布；活跃 Note/Head/Body/Tail 只读 immutable capsule，磁盘后续变化不混入当前实例。scanner/schema 57 owner、自动发现/导入、专用 managed mutation、external registration/capture 与整包 atomic reload/detach barrier 仍未接通。
...
- **整体仍不可用**：`SV1-1` 未完成；`SV1-2` 只新增已注册合法 managed folder 的生产选择窄链，没有 scanner/schema 57 owner、自动发现/导入、专用删改、external 或原子 reload；`SV1-3`～`SV1-7` 未实现。不能把这个运行时选择切片描述成 G1、Skin V1 或产品交付完成。
...
| G1 文件夹导入/选择/原子重载 | 部分实现、未交付；已注册合法 managed folder 的 production factory/选择自动 gate 已过，但 scanner/自动发现/导入、专用删改、external 与原子 reload/detach 未实现 |
...
| 6 | `SV1-2` G1 安全存储与原子重载 | **进行中**；已注册合法 managed folder 的 production factory/选择自动门已闭合；下一步是 schema 57 scanner owner/自动发现，再分别推进 managed mutation、external 与 atomic reload/detach，`SV1-3`～`SV1-7` 仍未完成 |
...
- 当前 managed folder selection 是“已注册记录可选择”，不是自动发现/导入；schema 57 scanner owner不存在，重启后目录可见性与记录reconcile尚未闭合。旧mutation入口虽已冻结，也不能替代专用no-follow journal/rollback服务。
...
2. 继续 `SV1-2`：下一刀先增加schema 57 nullable opaque scanner owner并接managed自动发现/reconcile；只允许完整稳定scan维护自身记录，null/unknown/另一authority及不完整scan均不得清理。随后专用managed mutation、external registration/capture与atomic reload/detach barrier各自独立过门；当前active实例对磁盘变化保持immutable而不会自动reload。
