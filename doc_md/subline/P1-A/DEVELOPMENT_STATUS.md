...
`SV1-0` 自动、schema 56 数据与用户实机 gate 已全部通过；`SV1-1` 已把用户选中的 managed `.osk` BMS 普通短键与长条头接入真实 gameplay 的安全静态图／连续编号帧路径，两刀自动 gate 均已通过，`V-001`、`V-002` 集中待验收。视觉待签收不再串行阻塞后续自动可证切片，但不得计为产品交付或阶段完成；下一切片冻结为 managed `.osk` BMS 长条尾静态图与编号帧动画，不提前改长条身状态/几何或作者 `Suppress`。

## 当前产品能力

- **恢复基线可用**：`.osk`/legacy mania、BMS F1 静态颜色/纹理/几何、选择链与程序化 `OmsSkin` 迁移 fallback 保持可用。
- **实现并自动验证的新增可见能力为 2**：selected managed package 可为 BMS 普通短键提供 `name-0`、`name-1`…编号帧动画，也可为 `NoteImage{lane}H`（含 `SH`/`S2H`）长条头提供静态图和同规则动画；普通短键静态 `NoteImage` 属恢复基线，不重复计数。
- **产品视觉签收为 0/2**：普通短键与长条头分别登记为集中视觉项 `V-001`、`V-002`，用户尚未签收，因此只能称实现/自动 gate 通过，不能称已交付功能。
- **安全回落已覆盖两个 critical slot**：selected 单槽缺失、损坏、空值、越权或超预算时逐组件回落；坏 head 不会拖垮同包有效 ordinary note，也不会从低层同名纹理拼件；异步换源只发布当前 revision 的完整结果。beatmap-local 优先目前只是注入式 provider-order 合同，不是真实 BMS `WorkingBeatmap` 能力。
...
| gameplay slot 三态 | 普通短键与长条头 critical slot 已消费 `Provide/Inherit`；作者 `Suppress` 与其它 slot 未交付 |
...
- `BmsLegacySkin` 继续叠加解析 `[Bms]` 并保留 `[Mania]`；native BMS 普通短键与长条头是当前仅有的真实 package 文件纵切。
...
| 4 | managed `.osk` BMS 普通短键与长条头视觉 | **`V-001`、`V-002` 集中待验收**；这是完成/release 声明门，不是后续开发开工门，不可复用静态恢复结论 |
| 5 | `SV1-1` 其余共同合同与玩家可见组件 | 下一切片冻结为 managed `.osk` BMS 长条尾静态图/编号帧动画；本刀仍不开放 `Suppress` |
...
- 产品自动验收扩为 **39/39**：在原普通短键矩阵上增加长条头静态图、真实 `DrawableBmsHoldNote → head → async host` 动画推进/循环、7K scratch、14K `S2H`、Note/head A→B、坏 head 与有效 ordinary note 隔离、低层同名纹理防串、provider-order、authority/file conflict 与异步换源。beatmap-local 项仍只是注入式 provider-order fixture。
- 合并态 BMS skin/runtime focused **248/248**，BMS full **1378/1378**；`osu.Desktop.slnf` Release **0 error / 11 known warnings**。保留 9 条 MessagePack 3.1.3 `NU1902` 与 BMS tests 既有 `CS8600`/`CA2007`，未使用 `NoWarn`。
...
- 普通短键与长条头视觉仍待用户集中确认；当前新增 LN 能力只到 head，不包含 body/tail、key、mania compatibility、完整 layout/G1/scene/script 或整包原子重载。
...
- 当前真实 package 纵切只覆盖 BMS 普通短键与长条头；单组件安全替换不等于整包/全 playfield 同帧原子 reload。
...
1. 将普通短键与长条头的观感、选择切换和 selected 坏包回落保持在[集中视觉清单](../../other/SKIN_V1_VISUAL_ACCEPTANCE_CHECKLIST.md)的 `V-001`、`V-002`，等待统一用户反馈；不得把自动测试写成视觉签收。
2. 实现 `SV1-1` 下一最小安全切片：managed `.osk` BMS optional `LongNoteTail` 静态图/连续编号帧动画；复用 exact revision、预算与异步发布，未声明仍为 `Inherit`，本刀不新增作者 `Suppress`，protected 默认 tail 继续透明。
