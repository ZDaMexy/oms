...
- **后续按完整campaign计进度**：`C1`先闭合external settings注册、Windows resolved-identity/no-follow capture、Realm、选择/重启、renderer及unregister；fresh-authoritative service-owned record只作provenance/admission，fresh held physical proof授权capture，paired exact capsule/logical manifest提供source，destination handles只授权写入。随后接通独立Import Managed Copy、managed rename与既有delete、journal支持及普通`.osk` archive安全；不得停在external DTO/registry/capture或thin stager。`C2`随后冻结并实现当前consumer的reload/owner协议，C3～C6新增consumer逐次加入，最终整包门到`C6`关闭。
...
- pre-C1当前runtime journal writer/schema仍为v2，大小预算因最多8193个节点的定长fingerprint manifest由128 KiB提高为1 MiB；最大manifest round-trip仍低于该界。pre-product阶段从无production delete caller，legacy-v1或旧v2 Delete intent若缺operation-derived tombstone、exact existing-record fingerprint、source-node manifest，或physical phase缺durable fallback disposition，不能安全迁移，strict load会按Invalid全局冻结且不猜测路径/record/tree。证据完整的v2 Rename/StagedImport/Delete及legacy-v1非Delete terminal处理保持旧合同；C1计划升级writer到v3但尚未实现。本切没有实现thin stager、任意path import、external、reload/detach、scene/script或canonical包。
...
2. 继续`C1`/`SV1-2`：先完成external只读settings注册/独立行级管理→capture→typed configured restart/selection→pure-Realm noncurrent unregister；Folder Skin Workspace的external行提供Open/Import Managed Copy/Unregister，managed行提供Open/Rename Folder，普通Realm `.osk`不进入列表；现有current Delete button/dialog保持唯一managed delete caller。所有action按committed record ID进入manager fresh重读，rename只改目录名。register/unregister commit、scanner/selection及rename/staged-import/delete共享有界exact external集合线性化；configured external retry延续`551a` typed startup/generic epoch合同。manager-owned Import Managed Copy只接受external record ID和用户明确target child，生成operation ID/staging path；fresh capture成对产出exact capsule与含empty directory的immutable logical manifest，文件bytes只从capsule复制，destination handles按manifest重建。首个destination write前建立single canonical v3 combined intent，覆盖copy/ProvisionalReady/既有move-publish并持久化external collision disposition；pre-C1 v2 schema冻结只读。ordinary `.osk`先做pre-open compressed length/bounded spool，再在bounded open后、任何`Filenames/GetStream/hash/Files.Add`消费前检查central-directory metadata，实际流硬计数并证明失败零残留。register/select/unregister不隐式复制，显式import永不修改外部源；全部managed mutation在final collision point复核exact Realm集合，否则`C1`不能退出。current unregister与当前consumer reload协议归`C2`，最终整包reload门到`C6`关闭；thin/arbitrary-path或foundation-only stager继续**NO-GO**。
