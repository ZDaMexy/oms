...
状态：进行中。三态 gameplay slot/fallback/precedence、semantic slot taxonomy、neutral lane identity、immutable lane topology projection/保持性 transition validation、configuration bucket explicit-presence、legacy mania 九个 primitive scalar、五组 indexed array 与四项 known global colour accepted-declaration snapshot、gameplay event envelope/order、capability negotiation/禁止 authority、六类 lane-resource neutral snapshot/BMS→mania 候选链，以及 internal 逐字段 resolution/revision-owner 十三个切片已完成；未接生产选择链，native context/revision/wire、full layout、完整 field-level config/production validation、具体 event payload/producer、真实 capability catalog/manifest/runtime 等条目仍待续。

1. 建立 neutral `GameplaySkinLayoutContext`、lane group/role/side/stable ID DTO 草案。**identity + topology projection/保持性 transition validation 已完成**：强类型 GroupId/LaneId、group/lane identity、side/role 与 immutable snapshot/group/entry 已固定；snapshot 显式携带 global/group-local logical/visual index、排序视图与强类型 lookup，process-local validator 要求 stable ID set、group logical index、lane membership/role/logical indices 不漂移并允许 side/visual reorder。internal BMS/mania fixture 固定 5K/7K 四 style、9K BMS/PMS neutral-shape 盲区、14K 双 deck 与 mania stage-local `SpecialKey`。完整 `GameplaySkinLayoutContext`、bounds/geometry、action/source/native context/revision/wire 与生产 adapter 接线仍待后续。
...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + legacy mania 九个 primitive scalar/五组 indexed array/四项 exact known global colour accepted snapshot + 六类 lane-resource field/resolution foundation 已完成**：实际 decoder 未产出 bucket/字段/index 时保持 `Absent`，显式空 bucket/资源名、显式默认 scalar、数组空/invalid item 接受值与显式 RGBA alpha 0 仍为 `Declared`；scalar/array/known-global-colour sidecar 在 decoder 成功转换/规范化时捕获 presence/value，之后 native mutation 不改变 provenance；短数组 synthetic tail 不提升为 declaration，boundary/gap index 不映射成 lane ID。note、LN head/body/tail、key up/down 被复制到绑定 immutable topology 的快照，source-aware reference 只有经 revision owner 构造、持有和基础验证后才可能成为 `Provide`。declaration provenance 仍不等于验证成功或 `Suppress`；per-column/任意扩展 colour、global resources/`NoteBodyStyle`、真实文件验证/materialization、malformed diagnostics、完整 neutral configuration/shared codec 与生产 adapter 仍待后续。
...
验收：前十三个合同切片已通过 capability 91/91、shared gameplay 总集 235/235（其中 event envelope/cursor 23/23、transition validator 12/12）、config presence shared/mania/BMS 5/5、13/13、9/9、legacy mania primitive scalar/indexed-array/known-global-colour snapshot 18/18、20/20、15/15（连同 decoder/declaration 65/65）、lane-resource shared/mania/BMS snapshot/candidate 12/12、6/6、29/29（合计 47/47），以及第九切 BMS resolution/owner 55/55；第十三切扩大回归通过 mania relevant 120/120、BMS relevant 108/108、core skin 57/62 同名既有失败与 `osu.Desktop.slnf` Release Rebuild 0 error / 20 warnings。额外 mania 全量 815/819 的 4 项仍是恢复基线同组 HoldNote auto-frame 期待失败。没有接入生产 `SkinManager`；第九切 BMS 全量 1117/1117 未在第十至十三切重跑。这不等于 `SV1-1` 整体完成；native context/revision/wire、full layout/geometry、per-column/扩展颜色、global resources/`NoteBodyStyle`、真实文件 validation/materialization/shared codec、concrete event payload/producer、真实 capability catalog/manifest/version/activation/runtime enforcement 与生产 adapter 接线仍待后续 fixture。
