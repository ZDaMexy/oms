...
| 4 | `SV1-2` G1 安全存储与原子重载 | **进行中** | 启动发现/选择及现有settings managed delete玩家可达；rename/import后端仍无caller/stager/UI，thin stager/caller当前NO-GO；下一候选是external完整产品纵切，current managed atomic reload/detach经2026-08-09产品链审计为NO-GO |
...
| --- | --- | --- | --- |
| `C1` 作者文件工作区与G1 UX | `SV1-2`前半 | external settings注册与独立registrations行级管理、strict Windows capture、service-owned Realm、选择/重启、BMS+mania具体artifact、Open Folder、noncurrent unregister；随后以fresh-authoritative service-owned record只作provenance/admission，并以fresh held physical root/ancestry、逐项no-follow identity与exact capsule/copy handles作为唯一byte authority，闭合external→fixed provisional stager/import UX、managed rename UI、既有delete。journal支持面须提供只读状态/稳定原因、脱敏diagnostic bundle、仅在证据可证明时的manager-owned恢复动作；不可证明时继续fail-closed并给出支持指引。同步补齐普通`.osk` archive importer的总压缩/解压字节、ratio、entry/depth/path、重复/歧义name、取消与失败清理/恶意archive gate | 作者可从settings注册只读源、选择/重启渲染、导入managed副本、rename/delete或解除注册；外部源永不被写改删；rename/staged-import/delete全部admission持有external physical proof到final collision point；freeze状态不会只显示“失败”而无安全支持路径；真实Windows、恶意archive、crash/cancel/restart/concurrency及宽回归通过。不得让owner token授权source bytes，也不得留下无caller后端、thin/arbitrary-path stager或仅registry/capture foundation |
...
| `C7` canonical双包、Authoring Kit与自动release收口 | `SV1-7`并汇总`SV1-1`～`SV1-7` | 交付可编辑、可复现构建的`oms-simple.osk`/`oms-complex.osk`、模板、完整schema/event/layout/capability/budget文档、validator/diagnostics与打包导入说明；发行物只读携带、完整性验证/原子恢复。canonical fallback接管必须覆盖`SkinManager`初始/current/config失败pair、ruleset providing containers、selection/reload失败回落、current managed delete/current external unregister、protected Realm record，并为升级时仍存在的delete journal v2/nonterminal durable fallback disposition提供旧`OmsSkin`证据继续恢复或显式版本迁移，之后才让程序化`OmsSkin`退出产品authority；canonical缺失/损坏必须阻止进入gameplay并进入明确安装修复，不能重新生成程序化视觉。第三方包、portable/custom-root/update、性能及全套自动门收敛 | 工程状态达到`SV1-1`～`SV1-7`“自动/合同/安全/release gate通过，人工待签收”；canonical切换前后的journal/recovery、delete/unregister receipt与失败回落均可证明收口，无程序化主题fallback、私有canonical特权、TODO validator/Authoring Kit或未归因自动失败；同时生成一键人工验收包，用户只需执行视觉/实机清单 |
...
7. **external registration/capture（下一完整产品候选）**：与受管目录mutation分离；external永久只读，只允许register/unregister，不提供物理rename/delete。只有settings目录选择caller、独立external registrations行级管理面、Windows no-follow完整capture、版本化service-owner Realm注册、exact-capsule factory/selection、配置重启、同一external capsule的BMS Note/LN与既有legacy mania note/hold最小production artifact、打开源目录及只解除注册能同切闭合时才GO；这不等于完整mania compatibility。每个管理行持有已提交record ID并提供Open Folder与Unregister，后者只对noncurrent启用；不得复用只绑定`CurrentSkin`的既有Delete按钮/dialog。service-owner token只授权管理Realm记录，不授权path/source bytes；selection与冲突判断仍须fresh held physical root/ancestry proof。注册不自动选择，active实例不持续读取live source，原地变化不自动reload。首个纵切只允许解除注册coherent current pair两半都不指向目标的noncurrent记录；任一半指向目标或pair split时settings必须禁用且manager稳定拒绝，用户须先显式选择并真实提交另一个skin。unregister本身不dispose任何旧`Skin`/capsule，也不宣称consumer detach或owner retirement。只有全部真实managed mutation admission都能把相关external held proof保持并复验到final collision linearization point，才可把合法service-owned非重叠external的全局阻断收窄为局部冲突；否则本纵切NO-GO并保留全局阻断。禁止只增加request/DTO/registry/capture backend后结束，也不得借本切预建reload barrier。
8. **整包原子 reload/detach（当前NO-GO）**：计划顺序仍在external registration/capture之后；只有代码证据证明managed路径可以独立闭合时才可例外提前。重新评估前产品必须先冻结真实触发方式、live gameplay等允许场景，以及BMS playfield/Note/LN/preview、core/mania drawable、菜单背景等全部consumer的参与、coherent publication、detach acknowledgement与owner retirement协议。获准后仍须fresh authoritative Realm/path/owner/freeze/capture/factory复核，完整准备immutable capsule与新skin instance，以generation/current-selection/revision gate发布；任一准备或publication失败保留exact旧pair/revision，全部consumer detach后才dispose旧owner，并覆盖latest-wins、reentrant、首个不可逆边界前取消、shutdown exact claim/reap/join及脱敏诊断。在真实caller与全consumer协议可同切闭合前，禁止manager-only reload API、强制同ID selection、逐组件`SourceChanged`拼接或预建无consumer foundation。
