...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + legacy mania 九个 primitive scalar/五组 indexed array accepted snapshot + 六类 lane-resource field/resolution foundation 已完成**：实际 decoder 未产出 bucket/字段/index 时保持 `Absent`，显式空 bucket/资源名、显式默认 scalar 与数组空/invalid item 接受值仍为 `Declared`；scalar/array sidecar 在 decoder 成功转换/规范化时捕获 presence/value，之后 native mutation 不改变 provenance；短数组 synthetic tail 不提升为 declaration，boundary/gap index 不映射成 lane ID。note、LN head/body/tail、key up/down 被复制到绑定 immutable topology 的快照，source-aware reference 只有经 revision owner 构造、持有和基础验证后才可能成为 `Provide`。declaration provenance 仍不等于验证成功或 `Suppress`；colour/global fields/`NoteBodyStyle`、真实文件验证/materialization、malformed diagnostics、完整 neutral configuration/shared codec 与生产 adapter 仍待后续。
...
验收：前十二个合同切片已通过 capability 91/91、shared gameplay 总集 235/235（其中 event envelope/cursor 23/23、transition validator 12/12）、config presence shared/mania/BMS 5/5、13/13、9/9、legacy mania primitive scalar/indexed-array snapshot 18/18、20/20（连同 decoder/declaration 50/50）、lane-resource shared/mania/BMS snapshot/candidate 12/12、6/6、29/29（合计 47/47），以及第九切 BMS resolution/owner 55/55；第十二切扩大回归另通过 mania relevant 120/120、BMS relevant 108/108、core skin 57/62 同名既有失败与 `osu.Desktop.slnf` Release 0 error / 20 warnings。没有接入生产 `SkinManager`；第九切 BMS 全量 1117/1117 未在第十至十二切重跑。这不等于 `SV1-1` 整体完成；native context/revision/wire、full layout/geometry、颜色/global fields/`NoteBodyStyle`、真实文件 validation/materialization/shared codec、concrete event payload/producer、真实 capability catalog/manifest/version/activation/runtime enforcement 与生产 adapter 接线仍待后续 fixture。
