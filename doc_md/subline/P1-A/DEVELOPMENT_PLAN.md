...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + legacy mania 九个 primitive scalar/五组 indexed array/四项 exact known global colour/exact per-column colour/十三项 bucket-global resource-name accepted snapshot + 两侧六类 lane-resource field/provenance/resolution foundation 已完成**：scalar/array/global-colour/per-column-colour/global-resource sidecar 在 decoder 成功接受时捕获 presence/value，之后 native mutation 不改变 provenance；exact `Colour{n}` / `ColourLight{n}` 可绑定 immutable topology，并由 mania `GlobalLogicalIndex`、BMS full `GlobalVisualIndex`、14K 双 deck 的 `Keys:8` `GroupLocalVisualIndex` 或 key-only 非 scratch visual enumeration 显式映射到 stable lane。十三项 global resource exact classifier 保留 explicit empty=`Declared`，unknown broad-prefix key 继续写 compatibility `ImageLookups` 但不进入 closed semantic snapshot。legacy mania 与 native `[Bms]` 的 note、LN head/body/tail、key up/down 均在 decoder 接受时写入 closed sidecar，factory 不再从公开可变 `ImageLookups` 反推；legacy 只接受严格零基 ASCII canonical token，native BMS 保留当前 raw lane token，不在本切猜测版本化迁移。declaration 仍不等于验证成功或 `Suppress`；任意扩展 colour/resource key、`NoteBodyStyle`、真实文件 validation/materialization、malformed diagnostics、完整 neutral config/shared codec 与生产 adapter 仍待后续。
...
验收：前十七个合同切片已通过 capability 91/91、shared neutral gameplay 总集 250/250（其中 event envelope/cursor 23/23、transition validator + topology revision owner 20/20）、config presence shared/mania/BMS 5/5、13/13、9/9、legacy mania scalar/array/global-colour/per-column-colour/global-resource snapshot 18/18、20/20、15/15、18/18、15/15（config aggregate **98/98**）、lane-resource snapshot/candidate 12/12、6/6、29/29 与 resolution/owner 55/55。第十五切的 mania/BMS per-column mapping 5/5、14/14 与旧 BMS candidate mapping 29/29 保持通过；第十六切 exact 十三项 focused **15/15**；第十七切 lane-resource accepted provenance shared/mania/BMS focused **21/21、6/6、40/40**。BMS full **1157/1157**，mania full **827/831** 仅恢复基线同名 4 个 HoldNote auto-frame 期待失败，core skin **57/62** 仍是恢复基线同名 5 项，Release Rebuild **0 error / 20 warnings**。保留 9 条 MessagePack `NU1902` 在 restore/build 重复显示和 BMS tests 既有 `CS8600`/`CA2007`，未使用 `NoWarn`；生产数据零写入，也没有文件验证、资源物化或 production lookup/renderer/`SkinManager` 接线。这不等于 `SV1-1` 整体完成；下一最小切片须先审计剩余 `NoteBodyStyle`、资源 validation/materialization 与其它未封闭配置面，full layout/geometry、production revision/event/wire、任意扩展颜色/资源键、shared codec、concrete payload/producer、真实 manifest/runtime enforcement 与 adapter 接线仍待后续 fixture。
