...
8. **`C1`作者文件工作区收口**：external子门通过后必须留在同一campaign。settings新增独立Folder Skin Workspace：external行持有committed record ID并提供Open Folder / Import Managed Copy / Unregister；exact scanner-owned managed行提供Open Folder / Rename Folder，普通Realm `.osk`不进入列表。所有动作只把record ID与必要用户输入交给manager并fresh重读；现有current Delete button/dialog继续作为唯一managed delete caller并复用`CanDelete`/`DeleteSkinAsync`，workspace不新增第二个delete入口。Unregister使用独立API/文案，Rename Folder只改direct-child且不复用会改展示名/`skin.ini`的旧popover。Import Managed Copy的target child由用户明确输入，manager生成operation ID/staging path；同一次fresh external capture成对产出exact capsule及含empty directory的immutable logical manifest，文件bytes只来自capsule，destination handle按manifest重建目录/文件，不重开external path。register/unregister commit与全部managed mutation共享有界exact external集合线性化，configured external restart延续typed startup/generic epoch合同。首个destination write前建立single canonical v3 combined intent，覆盖copy/ProvisionalReady与既有move/publish phase并持久化external collision disposition；v2永久冻结读取。ordinary `.osk`先做pre-open compressed length/bounded spool，再在bounded open后、任何archive内容消费前检查central-directory metadata，实际流硬计数。不得任意path、overwrite/merge/自动改名，也不得写改删external源。只有作者可完成注册→选择/重启→导入managed副本→rename→选择后经既有确认delete或pure-Realm解除注册的完整旅程，且malicious archive、crash/cancel/restart/concurrency与宽回归通过，`C1`才可关闭并进入`C2`。
