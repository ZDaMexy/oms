...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + legacy mania 九个 primitive scalar/五组 indexed array/四项 exact known global colour accepted snapshot + 六类 lane-resource field/resolution foundation 已完成**：scalar/array/known-global-colour sidecar 在 decoder 成功接受时捕获 presence/value，之后 native mutation 不改变 provenance；note、LN head/body/tail、key up/down 已投影到 immutable snapshot。已确认现有 lane-resource factory 在调用时仍读取公开可变 `ImageLookups`，因此只证明 snapshot 创建后不漂移，不能作为创建前 accepted provenance；下一切先做 exact per-column colour decoder-time sidecar/stable mapping，global resources 后续也须先补 image lookup sidecar。declaration 仍不等于验证成功或 `Suppress`；任意扩展 colour、`NoteBodyStyle`、真实文件 validation/materialization、malformed diagnostics、完整 neutral config/shared codec 与生产 adapter 仍待后续。
...
验收：前十四个合同切片已通过 capability 91/91、shared gameplay 总集 243/243（其中 event envelope/cursor 23/23、transition validator + topology revision owner 20/20）、config presence shared/mania/BMS 5/5、13/13、9/9、legacy mania scalar/array/global-colour snapshot 18/18、20/20、15/15（连同 decoder/declaration 65/65）、lane-resource snapshot/candidate 12/12、6/6、29/29 与 resolution/owner 55/55。第十四切新增 shared/BMS/mania publication 8/8、14/14、7/7；BMS full 1132/1132，mania full 822/826 仅既有 4 个 HoldNote auto-frame 期待失败，core skin 57/62 仍是同名既有 5 项，Release Rebuild 0 error / 20 warnings。没有接入生产 `SkinManager`。这不等于 `SV1-1` 整体完成；full layout/geometry、production revision/event/wire、per-column/扩展颜色、global resources/`NoteBodyStyle`、真实文件 validation/materialization/shared codec、concrete payload/producer、真实 manifest/runtime enforcement 与 adapter 接线仍待后续 fixture。
