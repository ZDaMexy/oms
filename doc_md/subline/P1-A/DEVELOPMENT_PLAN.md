...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + legacy mania 九个 primitive scalar/五组 indexed array/四项 exact known global colour/exact per-column colour/十三项 bucket-global resource-name/独立 `NoteBodyStyle` accepted snapshot + 两侧六类 lane-resource field/provenance/resolution + native `[Bms]` exact 22-colour snapshot foundation 已完成**：各 sidecar 都在 decoder 成功接受时捕获 presence/value，之后 native mutation 不改变 provenance；exact per-column colour 可绑定 immutable topology，十三项 global resource 与两侧六类 lane-resource 保持各自 closed classifier/raw token 兼容边界。`NoteBodyStyle` 保留当前 case-sensitive `Enum.TryParse` 接受的 named/numeric/undefined/comma value，但独立 snapshot 不读取 `[General] Version`；native colour 只记录 exact source key，comma-composite key 仅保留 public `Colours` compatibility 行为而不进入 closed provenance。declaration 仍不等于验证成功、slot `Provide` 或 `Suppress`；native BMS exact 12 geometry 等其余 closed fields、任意扩展 colour/resource key、真实文件 validation/materialization、package authority、malformed diagnostics、完整 neutral config/shared codec 与生产 adapter 仍待后续。
...
验收：前十九个合同切片中，第十九切 native `[Bms]` exact 22-colour focused 首次 **27/28** 的唯一失败为测试 `Count` 断言写法错误，修正并扩展后 **31/31**；BMS skin **332/332**、BMS full **1188/1188**，Release Rebuild **0 error / 20 warnings**，独立终审 blocker/major/minor **0/0/0**。本切未改 shared/mania/core 且未重跑该三组；上一切基线为 capability 91/91、shared neutral gameplay 250/250、shared config 126/126、mania skin 182/182/full 827/831 与 core skin 57/62，其中失败均为恢复基线已知项。保留 9 条 MessagePack `NU1902` 在 restore/build 重复显示和 BMS tests 既有 `CS8600`/`CA2007`，未使用 `NoWarn`；生产 Realm、`chartskin/`、用户皮肤目录与网络零访问、零写入，也没有文件验证、资源物化或 production lookup/renderer/`SkinManager` 接线。这不等于 `SV1-1` 整体完成；下一切按已完成审计推进 native `[Bms]` exact 12 geometry accepted provenance，完整 finite/range/screen-space validation 留给 neutral descriptor/solver。
