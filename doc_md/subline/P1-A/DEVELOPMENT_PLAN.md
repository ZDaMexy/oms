...
状态：进行中。三态 gameplay slot/fallback/precedence、semantic slot taxonomy、neutral lane identity、immutable lane topology projection/保持性 transition validation、configuration bucket explicit-presence、gameplay event envelope/order、capability negotiation/禁止 authority、六类 lane-resource neutral snapshot/BMS→mania 候选链，以及 internal 逐字段 resolution/revision-owner 十个切片已完成；未接生产选择链，native context/revision/wire、full layout、完整 field-level config/production validation、具体 event payload/producer、真实 capability catalog/manifest/runtime 等条目仍待续。
...
5. neutral config 保存 explicit declaration/presence；legacy 自动合成的默认 bucket 不得误判为 `Provide`。**bucket + 六类 lane-resource field/resolution foundation 已完成**：实际 decoder 未产出 bucket/字段时保持 `Absent`，显式空 bucket/资源名仍为 `Declared`；note、LN head/body/tail、key up/down 被复制到绑定 immutable topology 的快照，source-aware reference 只有经 revision owner 构造、持有和基础验证后才可能成为 `Provide`。declaration provenance 仍不等于验证成功或 `Suppress`；scalar/array/colour/global fields、真实文件验证/materialization、malformed diagnostics、完整 neutral configuration/shared codec 与生产 adapter 仍待后续。
...
验收：前十个合同切片已通过 capability 91/91、shared gameplay 总集 235/235（其中 event envelope/cursor 23/23、transition validator 12/12）、config presence shared/mania/BMS 5/5、13/13、9/9、lane-resource shared/mania/BMS snapshot/candidate 12/12、6/6、29/29（合计 47/47），以及第九切 BMS resolution/owner 55/55；第十切另通过 mania relevant 120/120、BMS relevant 108/108、core skin 57/62 同名既有失败与 `osu.Desktop.slnf` Release 0 error / 20 warnings。没有接入生产 `SkinManager`；第九切 BMS 全量 1117/1117 未在第十切重跑。这不等于 `SV1-1` 整体完成；native context/revision/wire、full layout/geometry、六字段以外的完整 config、真实文件 validation/materialization/shared codec、concrete event payload/producer、真实 capability catalog/manifest/version/activation/runtime enforcement 与生产 adapter 接线仍待后续 fixture。
