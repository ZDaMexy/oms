...
P1-A 负责 Skin V1、共享皮肤产品面与 release gate。`C1` 已闭合Folder Skin Workspace、external只读注册/选择/configured restart/noncurrent Unregister、managed Open/Rename/Delete、single-v3 ManagedCopy/exact-set recovery与ordinary `.osk` bounded ingress/zero-residue receipt；`C2` 已闭合Settings唯一manual Reload、live gameplay/preview prepare前拒绝、三源same-ID publication、完整participant/holder生命周期、current external/managed/ordinary mutation与legacy旁路禁用。当前为`2/7 closed，C3 active`，转入P1-K lane/keymode前置与唯一gameplay layout。`V-001`～`V-004`仍为0/4；G1最终整包门、`SV1-1`、`SV1-2`整体、Skin V1与release均未完成，程序化`OmsSkin`仍是迁移链底。
