...
3b. lexical preflight不是安全打开或mutation capability，pure capsule也不是其替代品：`File.GetAttributes()`存在TOCTOU，capsule只冻结producer实际交付的bytes，无法单独证明这些bytes来自preflight root、同一物理文件或一次稳定捕获。production受管目录factory必须只消费resolver-issued request经Windows native capture完整成功返回的capsule；scanner现有held-root gate只闭合发现/reconcile，rename/import/delete仍须各自独立的no-follow identity与恢复gate，不得直接拿normalised path执行I/O。
...
3j. 在任一专用managed端到端操作落地前，folder的editor/export/rename/delete/undelete/file add-delete-replace/update-import/external-edit等旧Realm/package入口一律冻结。任何通用mutation即使经base/interface dispatch也必须在真正Realm事务内按ID重新获取authoritative记录后再判定；调用方传入的detached、stale或同ID伪造`SkinInfo`字段永远不能授权，importer也必须在清空文件前于同一事务复核。当前公共mutation authority/recovery foundation不含物理写primitive或Realm新记录publisher，不能被旧入口消费；该foundation也不是managed rename/delete/import已实现。
...
4d. foundation完成本身不开放UI或物理写入；端到端操作按rename → staged import → delete独立过门，每项分别验证journal crash points、重试幂等、collision/no-follow、Realm与磁盘一致性、selection竞态、隐私和取消。
