...
9. neutral config 必须保留 explicit declaration/presence：presence carrier 的默认值是 `Absent`，显式 bucket 即使为空也为 `Declared`；declaration 只表示来源事实，不等于 slot `Provide`、验证成功或 `Suppress`。通用 carrier 必须区分显式 `false`、`0`、空字符串与缺失。legacy mania primitive scalar/indexed-array/known-global-colour sidecar 必须在 decoder 成功转换、规范化或颜色解析时同时捕获 presence 与 accepted value，不得从 synthetic default 或之后可变的 native public 值反推；数组必须逐 index 保存，短数组未出现尾部为 `Absent`，空/invalid item 依既有兼容规则为 `Declared(0)`。当前十三切只覆盖九个 primitive scalar、五组 indexed array、四个 exact known global colour 和 note、LN head/body/tail、key up/down 六个逐 lane 资源字段；per-column/任意扩展颜色、global resources/`NoteBodyStyle` 仍未进入完整 field-level neutral config。
