...
3h. managed folder selection只允许authoritative Realm记录同时满足`IsManaged`与resolver/native capture/factory合同。managed请求必须在update thread进入且在线程门通过前不得递增generation或取消既有pending；普通Realm `.osk`保留既有线程兼容。所有请求的generation bump与Realm commit、managed completion的最终generation检查与commit必须串在同一commit lock内，以generation/cancellation异步准备；capture完成后与factory完成后须双重复核当前authoritative记录，prepared target还须匹配同一对象identity。只有current selection/generation仍一致时才一次提交`CurrentSkinInfo`/`CurrentSkin`；invalid、未注册/unmanaged、external、capture/factory异常、stale、mutation race、reentrant请求或completion scheduler fault均保留旧pair，稳定失败reason不得被后到completion覆盖，provisional owner必须清理且任务fault不得泄露路径或成为unobserved exception。
...
8. 删除current skin前必须等待并确认当前已验证的protected fallback selection pair真正提交，不能只向scheduler排队赋值后立即删除；canonical接管前该authority为程序化`OmsSkin`，接管后为只读`oms-simple.osk`。切换失败、无法确认提交或恢复状态不明时拒绝物理删除；在确认式切换路径落地前current受管目录delete保持冻结。external“删除”只能解除注册，不能写、改名或删除物理目录。
