...
3b. lexical preflight 不是安全打开或 mutation capability，pure capsule 也不是其替代品：`File.GetAttributes()` 存在 TOCTOU，capsule 只冻结 producer 实际交付的 bytes，无法证明这些 bytes 来自 preflight root、同一物理文件或一次稳定捕获。任何 production folder factory、inventory、scanner、rename/delete 接线前必须先以 Windows native fixed-handle、handle-relative no-follow capture 建立稳定 inventory/final identity，再创建 capsule；不得直接拿 normalised path 执行 I/O。
3c. capsule content revision 只表示规范资源名、长度和实际读取内容的确定性身份，不是 authority、physical/capture identity、semantic package validation、`InstantiationInfo` 或选择资格。精确 declared length 也不能证明同长度内容未在读取中变化；8.3/SUBST/junction/hardlink/alias 与读取/枚举竞态必须由 native capture 单独闭合。
3d. capsule 必须独占并清理 backing；resource view 非 owning，`Get`/`GetStream` 返回 defensive copy，view dispose 不得释放 capsule。失败/取消不得发布半成品且必须清理 provisional backing；active capsule 只能在所有 consumer detach 后退役。
3e. folder factory 必须走 exact-capsule 构造路径，不得让 `Skin` 默认添加的 live `RealmBackedResourceStore` 排在 capsule 前面，否则会破坏 exact-revision authority。普通 `.osk` 与 SkinEditor 当前依赖 Realm live store/refresh，不得因 G1 folder capsule 被全局冻结；其原子更新另走 prepared revision/new-instance 协议。
