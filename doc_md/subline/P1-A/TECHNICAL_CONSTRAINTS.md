...
9. neutral config 必须保留 explicit declaration/presence：presence carrier 的默认值是 `Absent`，显式 bucket 即使为空也为 `Declared`；declaration 只表示来源事实，不等于 slot `Provide`、验证成功或 `Suppress`。通用 carrier 必须区分显式 `false`、`0`、空字符串与缺失；当前前十切仍只覆盖 note、LN head/body/tail、key up/down 六个逐 lane 资源字段，不能写成完整 field-level neutral config 已完成。
