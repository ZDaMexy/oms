...
3b. lexical preflight 不是安全打开或 mutation capability，pure capsule 也不是其替代品：`File.GetAttributes()` 存在 TOCTOU，capsule 只冻结 producer 实际交付的 bytes，无法单独证明这些 bytes 来自 preflight root、同一物理文件或一次稳定捕获。任何 production managed folder factory 必须只消费 resolver-issued request 经 Windows native capture 完整成功返回的 capsule；inventory、scanner、rename/delete 仍需各自 no-follow/identity gate，不得直接拿 normalised path 执行 I/O。
