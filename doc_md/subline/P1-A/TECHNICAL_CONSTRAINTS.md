...
10d. 六类逐 lane resource 的 decoder provenance 仍有两个独立 mutable window：`LegacyManiaGameplaySkinLaneResourceSnapshotFactory` 在调用时读取可替换的 public `LegacyManiaSkinConfiguration.ImageLookups`，`BmsGameplaySkinLaneResourceSnapshotFactory` 同样读取 public mutable `BmsSkinConfiguration.ImageLookups`。两者当前都只能证明 snapshot 创建后不漂移，不能证明 decode 后、factory 前的 mutation 不会伪造或擦除 declaration。下一切必须分别让 legacy mania compatibility 与 native `[Bms]` factory 只读各自 decoder-time accepted sidecar；若分成两个切片，在两侧都完成前不得宣称 lane-resource durable decoder provenance 已闭合，也不得从任一 mutable dictionary 反推完整 V1 config。
