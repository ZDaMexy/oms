...
10c. 现有 `LegacyManiaGameplaySkinLaneResourceSnapshotFactory` 在 factory 调用时读取公开可变 `ImageLookups`；已创建 snapshot 虽不可变，但 decode 后、factory 前的 mutation 仍可伪造或擦除 declaration。因此它只能作为 compatibility projection fixture，不能被描述为 durable decoder provenance。下一切须先为 exact `LightingN`、`LightingL`、`StageLeft`、`StageRight`、`StageBottom`、`StageLight`、`StageHint`、`Hit0`、`Hit50`、`Hit100`、`Hit200`、`Hit300`、`Hit300g` 建立 decoder-time accepted sidecar，随后硬化六类 lane resource 的 `ImageLookups` provenance；禁止从 `CustomColours`/`ImageLookups` mutable dictionary 反推完整 V1 config。
