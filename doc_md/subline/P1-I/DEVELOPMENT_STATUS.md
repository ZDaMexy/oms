...
- **最近一次验证（2026-06-22）**：选曲展示/筛选增强三连（难度表归类 / 「显示转谱」三态 / mania 难度表分组）——`BeatmapLocalMetadataDisplayResolverTest` **7/7**（+4 归类：单表/多表 sort order/Unrated 空/非 BMS 空）、`FilterMatchingTest` **98/98**（+4 三态：Hidden 只原生/Shown 原生+转谱/ConvertedOnly 隐原生留转谱/bool 投影）、`BmsConvertedDifficultyTableGroupingTest` **4/4**、`ManiaDifficultyTableGroupingTest` **3/3**（端到端丢原生 mania + 转谱按表分组 + 未收录进 Unrated）、carousel sort+group **25/25** + `BeatmapCarouselFilterGroupingTest` **17/17** 不回归；`osu.Desktop.slnf` Release **0 错误**。**用户 2026-06-22 实机分三次验收（难度表归类「暂时未见异常」/ 三态转谱「实机验证无异常」/ mania 难度表分组「经观测暂时无异常」），三项均验收通过。** 约束见 [TECHNICAL_CONSTRAINTS.md](TECHNICAL_CONSTRAINTS.md) #19/#20/#21，详见 [CHANGELOG.md](CHANGELOG.md) 2026-06-22（其二/其三/其四）。（上一轮 2026-06-21 carousel「自动跳滑」修复见 CHANGELOG 2026-06-21 + 约束 #17/#18。）
