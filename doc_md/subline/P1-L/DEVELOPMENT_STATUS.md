...
- **已落地链路**：
  ...
  - BGA 浮窗默认尺寸 ~75% + 顶角锚（右→右上等），避开 playfield 与顶部 HUD。
- **产品决策（已落地）**：图序列 + 视频一起做；默认镜像对侧（居左→右/居右→左/居中→右/14K DP→中缝）；letterbox（`FillMode.Fit`）；皮肤可定制控件（`BgaPanel`）+ 接口预留；仅 native 路径、converted-mania 不在 v1。
- **验收**：视频 BGA（.mp4/H.264）实机播放已用户确认 ✅（2026-06-14）；剩图序列/POOR/14K 中缝/seek 同步逐谱抽验交接人工。overlay"黑透" + `#ARGB` 当前为加色近似，保真后续。
