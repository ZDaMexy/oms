...
- 星轨总体观感已有明确否定，双包修改按用户要求留待新对话；同步当前发行入口与 P1-A 结论，保留既有包、工程验证日期和数据事故边界。此次仅文档变更，没有构建、重新打包、安装验证或新的发行签收，不改旧集中目录中的生成快照；详见 [P1-A 状态](../P1-A/DEVELOPMENT_STATUS.md)。

### 启动提示修复版发行与真实跨版本更新

- 当前实际交付更新为 `oms_20260911_startup-fix-final.zip`（344,240,108 B）和 `oms-skin-c7-acceptance-20260911-startup-fix-final`；完整来源身份见 [STATUS](DEVELOPMENT_STATUS.md)。真实 ZIP 解压、只读原件与完整清单核对通过，PS5、无 Git/SDK 的实际组装通过且来源不变。本轮部署入口和稳定发行合同未改；`_4`、`preview-fix` 结果保留为历史。
- 新包首次便携、自定义保存位置、坏工作副本恢复及同包覆盖后启动均完成稳定运行和正常退出，无强制终止；四轮结束后的两处测试根额外只读副本确认 BMS、mania 可用，仍不冒充四份独立时点快照。
- 另从旧 `preview-fix` 安装独立副本完成真实跨版本更新：覆盖工具前后用户文件、数据库和 portable 标记保持原样，两款旧只读原件留在备份；覆盖后工作副本仍为旧版，由游戏首次启动自动从 `f32ae2b8…` 换为 `532224fb…`，随后正常退出。主升级记录的玩法检查 Pending 保留原样，后续独立只读副本已确认两玩法可用，见 `artifacts/skin-startup-warning-20260911/final-delivery/rulesets-upgrade.log` 及其引用记录。
- 旧测试安装、旧发行来源、新发行来源及三处受保护既有保存根前后字节和属性均相同，未打开原数据库；这只证明本轮受保护运行的边界，不能倒推首次事故无损。首次事故缺少事前快照的事实继续保留。
- 随包作者程序与本轮普通制作路径实际演练版本一致。V-001～V-004 0/4、V-005 与 C7 全部人工门仍未签收，非便携新账户/虚拟机仍待人工运行；简洁款为正式默认与保底，复杂款仅展示和默认候选，不宣布整个 Skin V1 或发行版本完成。最终制品独立复核已通过，文档及提交记录统一见 C7 报告。
