...
- [build-release.ps1](../../../build-release.ps1) 产出完整自包含多文件 ZIP（`PublishSingleFile=false`）。本轮 BMS 预览修复包为 `release-repo/oms_20260911_preview-fix.zip`，完整解压后直接运行 `osu!.exe`，无需另装 .NET；玩法 DLL 和安装原件留在真实安装目录。
...
- 本轮作者工具、完整双包源文件、普通导入包、模板和验收工具均随真实发行物交付。最终可直接运行的集中验收目录为 `release-repo/oms-skin-c7-acceptance-20260911-preview-fix`；已用该真实包在 PS5、PATH 不含 Git/SDK 的进程中完成组装，来源字节及属性不变。随包作者程序与独立完成制作演练的版本摘要一致，见 [WORKSHOP](../../../skin-authoring/docs/WORKSHOP.md)。
...
最终 ZIP SHA256 为 `1c1e11f4f31b2c57308c75558fd403b2fa84aa4083698af02d6a126a82513fd3`，发行清单 SHA256 为 `9df5370ac24b5fe8ddd4a368ae93f6d9675c4fdf39646202e03617c27eef3ed7`。`artifacts/skin-preview-entry-20260911/release-extracted-extraction.json`、`artifacts/skin-c7-evidence/release-startup-preview-fix/results.json` 与 `artifacts/skin-preview-entry-20260911/acceptance-assembly.json` 固定本次来源；四轮实际运行时间为 UTC 12:02:24～12:04:57，均 `NormalExit=true`、退出码 0、无强制终止，原发行来源保持不变。两处共享测试根的只读玩法检查见 `artifacts/skin-preview-entry-20260911/rulesets-portable.log` 与 `artifacts/skin-preview-entry-20260911/rulesets-custom.log`。完整开发证据与自动检查边界集中在 [C7 验证记录](../../other/SKIN_SYSTEM_C7_VALIDATION_20260909.md)。
