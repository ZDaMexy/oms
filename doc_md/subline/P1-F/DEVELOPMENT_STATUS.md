...
- `P1-F` 已完成子线建档。
- `OsuGameDesktop.CreateStorage()` 已按 `portable.ini -> data/` 接通便携数据根；当前正式打包入口已明确为 `build-release.ps1 -> release-repo/oms_YYYYMMDD(.zip)`，发行根目录保持 `osu!.exe` + `portable.ini` + `lazer.ico` + `beatmap.ico` + 中英双语 `how to update.txt`。
