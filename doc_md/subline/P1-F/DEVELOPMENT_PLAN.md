...
1. 保持打包脚本、文件名、根目录内容与 `IncludeAllContentForSelfExtract=true` 不回退；先给脚本生成的中英 `how to update.txt` 补齐保留 `storage.ini` 的说明，并与实际基础数据根中的重定向文件位置一致。
