...
- pre-start 1 号普通轨纯视觉流速预览已完成 feasibility review：实现路径应为第一非 scratch 轨上的纯视觉 preview layer，复用当前 note skin lookup 与 scroll authority；不得经 `BmsHitObject` / `DrawableBmsHitObject` / `BmsLane` 判定或键音链。
