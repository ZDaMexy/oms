...
状态：R0/R1 已解除；三态 slot/precedence、内部 semantic taxonomy、neutral lane identity/topology/保持性 transition validation、config bucket explicit-presence、event envelope/order、capability negotiation/禁止 authority、六类 lane-resource candidate plan 与 internal resolution/revision-owner 十个切片已完成，其余共同合同待续。
