...
状态：R0/R1 已解除；三态 slot/precedence、内部 semantic taxonomy 与 neutral lane identity primitives 三个切片已完成，其余共同合同待续。

1. 冻结 ruleset-neutral ini codec、layout context、lane group/role/side/stable ID 和 mania compatibility fixtures；lane/group 强类型 ID、side 与 Key/SpecialKey/Scratch identity primitives 已落，layout aggregate、索引和真实 adapter fixture 待续。
