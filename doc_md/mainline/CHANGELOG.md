...
### Skin V1 当前事实同步与文档治理交接（P1-A）

在不改代码、产品行为、生产数据或 gate 结论的前提下，把 mainline/P1-A 路由、作者文档、恢复证据与 `.Codex/memory` 统一到首个玩家可见纵切后的真实状态：`SV1-0` 全过，Skin V1 新增可见功能为 1，新动画实机仍待确认，完整 V1 仍不可用；同时纠正“当前选择是 managed 包”的歧义并脱敏皮肤恢复记录中的本机绝对路径。实现暂停于 `d1ea483`，下一新对话先做文档与 memory 健康治理，治理不计产品进度；完成并重新冻结执行门后才闭合动画实机 gate，并决定下一组件。完整记录见 [P1-A CHANGELOG](../subline/P1-A/CHANGELOG.md)。
