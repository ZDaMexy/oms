...
> 最后更新：2026-06-23（**P1-K K12 修复落地**：确诊并修复 `BMS→mania` 转谱星数被 BGM/scratch sample-only 对象灌高（难度入口 nested-aware 过滤 + `conversion_version` bump，pre-fix 反证 +113%，BMS 961/961 + Release 0/0），并更正 P1-K 四件套失真叙述，见下「P1-K 子线状态」。此前 2026-06-22 **P1-I 选曲展示/筛选三连**：标准面板加难度表归类标签 + mania「显示转谱」二态升三态 enum + mania 新增「难度表」分组（只显示 BMS 转谱）；`osu.Desktop.slnf` Release 0 错误、用户实机分三次验收通过）。当前真实状态见下「最新快照」，最新验证快照见「最近一次验证」，历史切片见 [CHANGELOG.md](CHANGELOG.md)——按本页规则「最近一次验证只保留最新一条；历史归 CHANGELOG」，抬头不再堆叠历史快照。
