...
| BMS 全量测试 | **1002/1002** | 最近一次全量 `osu.Game.Rulesets.Bms.Tests`（2026-06-29 P1-A `F1` 皮肤主面完成 + 验收 + `G1` 刀①后重跑：颜色/纹理/几何三轴皮肤化 + reference 验收 + folder-backed 直读建块；含 BGA Phase 5/5.1/5.2 + 判定 parity 29 项。注：1 项 BGA 缓存 temp 清理测试间歇 flaky·`git stash` 干净树同样失败·与皮肤工作零因果·归 P1-L 跟踪） |
