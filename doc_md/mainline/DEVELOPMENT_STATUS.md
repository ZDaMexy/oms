...
> 最后更新：2026-06-14（**判定 parity 第 1–2 刀**：第 1 刀建判定窗口 parity 契约测试 `BmsJudgementSystemParityTest` 29/29 + 统一跨家族边界（行为不变）；第 2 刀从 beatoraja `JudgeProperty.SEVENKEYS` 溯源后**修复一处真实 bug——beatoraja BAD 早/晚非对称方向写反**（应早 280/晚 220，**行为变更**），并把 IIDX empty-poor `500/150` 与 CN release 收口为 documented heuristic。校正现状＝`BEATORAJA`/`LR2`/`IIDX` parity 并非「全缺」、窗口数值大量已溯源；剩余仅第 3 刀把区分接进 gameplay 反馈。详见下「最近一次验证」与 [P1-C](../subline/P1-C/)）。此前 2026-06-10（转谱性能审查：落地两项**行为不变**的库级冗余消除——① 转谱器移除 mania 星级自算，星级唯一归 `ManiaDifficultyCalculator`（消除持久化路径双重 strain、加载期无谓计算；新增约束 [P1-K CONSTRAINTS K9 #17](../subline/P1-K/TECHNICAL_CONSTRAINTS.md)）；② keysound 样本按 WAV 槽号 memo。暂缓 `buildEventTimeline` 重写（待 profiling）。转谱 18/18 + BMS 871/871 + Release 0 错；详见 [CHANGELOG](CHANGELOG.md) 2026-06-10。此前 2026-06-08：转谱-mania「按 key1 触发 bgm1 / 长音重叠 / 暂停不停」**已解决**——真因 = mania 按键音效反馈 `GameplaySampleTriggerSource`，修复 = 转谱器把 BGM/scratch 的 `Samples` 置空、键音走 store 的 `KeysoundSample`（用户多谱实测 ✅）；2026-06-07 J6 已修长 BGM 被 store 通道偷断（floor 128）+ tap-note→store per-WAV cut。转谱键音重复的 LN 部分、dense D 卡顿、选歌预览 Track 泄漏仍后置。详见 [P1-J 四件套](../subline/P1-J/)）
...
| 桌面端构建 | 通过 | `dotnet build osu.Desktop.slnf -p:Configuration=Release` 0 警告 0 错误（2026-06-08） |
| BMS 全量测试 | **916/916** | 最近一次全量 `osu.Game.Rulesets.Bms.Tests`（2026-06-14，含判定 parity 契约 29 项） |
...
### 2026-06-14（P1-C：判定 parity 第 1–2 刀——契约测试 + 跨家族边界统一（行为不变）+ beatoraja BAD 早/晚非对称方向修复（行为变更））

- **背景校正**：通读判定链路后确认 `BEATORAJA` / `LR2` / `IIDX` parity **并非「全缺」**：基类已预留 early/late / scratch / LN-release / excessive-poor 全部钩子；IIDX `16.67/33.33/116.67/250` 与 LR2 四档窗口已与 [../other/IIDX_REFERENCE_AUDIT.md](../other/IIDX_REFERENCE_AUDIT.md) 逐项吻合，beatoraja 已有整数截断缩放 + 非对称 BAD + scratch/release profile，excessive/empty poor 已按家族参数化（LR2 仅 note 前）。
- **第 1 刀（G1+G2，零行为风险）**：① [BeatorajaJudgementSystem.cs](../../osu.Game.Rulesets.Bms/Scoring/BeatorajaJudgementSystem.cs) 的 `Evaluate` 边界统一加 `+ BoundaryEpsilon`，跨家族结论一致；② 新建 [BmsJudgementSystemParityTest.cs](../../osu.Game.Rulesets.Bms.Tests/BmsJudgementSystemParityTest.cs) 把窗口边界 / LR2 四档 / beatoraja 缩放档与非对称 / scratch·release 扩窗 / excessive-poor early-late 真值表 / 跨家族 inclusive 边界锁成 parity 契约。
- **第 2 刀（G3 行为变更 + G4 收口）**：从 beatoraja 源码 `JudgeProperty.SEVENKEYS` 溯源得 note BAD `{…,-280000,220000}`µs（早 280/晚 220，早窗更宽），发现 OMS 把 BAD 早/晚**写反**（早 220/晚 280），已改回（scratch 290/230、LN release 280/220）；beatoraja 早击至 280ms 仍 BAD、晚 BAD 收紧到 220ms。属性显示 `BAD hit window -早/+晚` 修后自动正确。IIDX empty-poor `500/150` 与 CN release 因 IIDX 闭源无权威单值，收口为 documented heuristic（不宣称 parity）。三处既有锁旧值测试按新值更新。约束见 [P1-C CONSTRAINTS #14–#17](../subline/P1-C/TECHNICAL_CONSTRAINTS.md)。
- **测试 / 构建**：`BmsJudgementSystemParityTest` **29/29**、`osu.Game.Rulesets.Bms.Tests` 全量 **916/916**、`osu.Desktop.slnf` Release **0 错误**（生产代码 0 警告）。
- **更早快照**（2026-06-13 Random/Mirror 重排重复应用修复 + 自定义 pattern UX、converted-mania 键数显示、选歌曲名/难度名展示；2026-06-11 转谱游玩帧抖动 + 开局 gen2 冻结双修；2026-06-08 转谱「按 key1 触发 bgm1」真因＝mania 按键音效反馈、用户多谱实测 ✅；2026-05-31 难度表全 Unrated 真根因＝共用 `RulesetData` 列互相覆盖）均已归档 [CHANGELOG.md](CHANGELOG.md)。本状态页只保留最新一条快照。
...
| P1-C 判定语义与反馈闭环补强 | BEATORAJA / LR2 / IIDX parity / FAST/SLOW / judge display / BMS 结果页反馈面 / visual timing-offset / EX pacemaker / 权威 GN 与调速反馈 / pre-start 1 号普通轨纯视觉流速预览 | 已阶段性收口（feedback family、tri-mode/pre-start 链与 results-side consumer proof 已落地；**判定 parity 第 1–2 刀已收口**：窗口/非对称/excessive-poor 已溯源并由契约测试锁定、修复 beatoraja BAD 方向写反，剩 gameplay 反馈区分第 3 刀；richer judge display 后置 backlog） |
| P1-J BMS gameplay runtime 性能与音频时序治理 | shared keysound pool 时序 / dense-lane hot path / live channel resize 安全合同 / dense full autoplay replay 分流 | 进行中（`J1` / `J4` 已完成，`J2` / `J3` 首刀已落地，`J5` 自动化已闭合；full autoplay 专用 replay 分流与 keysound 预热（2026-06-11 起含玩家模式）已落地，once-per-run hitch 已确诊为开局阻塞 gen2 并修复（2026-06-11 用户实测 ✅），剩余人工验收；J6 转谱音频已闭合 E（暂停停 BGM）/ 长 BGM 通道偷断（store floor 128）/ tap-note→store per-WAV cut（生产默认）/ bgm1 按键触发（BGM/scratch `Samples` 置空，2026-06-08 用户多谱实测 ✅）/ tap-note 池化（2026-06-10 经接口路由 store）/ 游玩期帧抖动（2026-06-11 同样本快路径，用户实测 ✅）/ 开局 gen2 冻结（2026-06-11 prewarm 放开玩家模式）；转谱键音重复的 LN 部分、50k 极端 dense（未 profile）、选歌预览 Track 泄漏仍后置） |
