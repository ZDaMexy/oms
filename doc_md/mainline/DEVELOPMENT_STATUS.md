...
> 最后更新：2026-06-18（**P1-I I5–I7**：BMS 选歌「展示层级」下拉 + 层级返回条 + 难度表分组解析缓存落地，并修复层级展开态/缩进与「非 BMS 播放谱面进入 BMS 误展开分组」；BMS 全套 918/918、人工实机验收通过）。当前真实状态见下「最新快照」，最新验证快照见「最近一次验证」，历史切片见 [CHANGELOG.md](CHANGELOG.md)——按本页规则「最近一次验证只保留最新一条；历史归 CHANGELOG」，抬头不再堆叠历史快照。
...
### 2026-06-15（P1-L BGA Phase 5/5.1：BMS 游玩 BGA 背景图·动画·视频链路落地——自动化通过、人工视觉验收待办）

- **Phase 5（2026-06-14）**：修复 native BMS 此前无真正 BGA——解析层（P1-K）已产出完整 BGA 事件/定义，但转换层只取一个静态 `metadata.BackgroundFile`、时间线被丢弃，显示层 `BmsBackgroundLayer` 静态占位件且被不透明 lane 背板完全遮挡。落地：转换携带 `BmsBeatmap.BgaTimeline`（照 `Mines` 不进 `HitObjects`），运行时 `BmsBgaPlayer` 在皮肤可定制浮窗 `BgaPanel`（挂 `DrawableRuleset.Overlays`、不被遮挡）按时间线合成图序列 + 视频（osu!framework FFmpeg `Video` + 时钟同步，资源经 `WorkingBeatmap.GetStream` 直读 `chartbms/`），POOR 层按 `#POORBGA` 在 miss flash，默认镜像 playfield 布局 + letterbox，`ShowBga` 开关，无 BGA 回退静态图，仅 native 路径。
- **Phase 5.1（2026-06-15）**：框架捆绑 FFmpeg 打不开的老式 `.mpg/.wmv/.avi/.flv` 经 opt-in 外部 ffmpeg（用户自备、OMS 不分发）后台转 H.264 `.mp4` 缓存（`BgaVideoTranscode` 开关；无 ffmpeg / 关闭 / 失败＝静态图回退，无回归）。
- **测试 / 构建**：`osu.Game.Rulesets.Bms.Tests` 全量 **946/946**（BGA 新增 17 + 13）、`osu.Desktop.slnf` Release **0 错误 0 警告**；实机逐谱人工视觉验收交接人工。
- **更早快照**（2026-06-14 判定 parity 第 1–2 刀：契约测试 `BmsJudgementSystemParityTest` 29/29 + 跨家族边界统一（行为不变）+ 修复 beatoraja BAD 早/晚方向写反（行为变更，应早 280/晚 220）；2026-06-13 Random/Mirror 重排重复应用修复 + 自定义 pattern UX、converted-mania 键数显示、选歌曲名/难度名展示；2026-06-11 转谱游玩帧抖动 + 开局 gen2 冻结双修；2026-06-08 转谱「按 key1 触发 bgm1」真因＝mania 按键音效反馈、用户多谱实测 ✅）均已归档 [CHANGELOG.md](CHANGELOG.md)。本状态页只保留最新一条快照。
...
| 1.1.12 测试矩阵与 release gate | 进行中 | Mania skin 92/92、BMS 聚焦 111/111、osu.Game.Tests 18/18 已复核；BMS 全量 **946/946** 已于 2026-06-15 复核（见上「开发指标」），mania 全量与 scratch bridge 继续沿用 2026-04-24 快照 |
