...
> 最后更新：2026-06-08（转谱-mania「按 key1 触发 bgm1 / 长音重叠 / 暂停不停」**已解决**——真因 = mania 按键音效反馈 `GameplaySampleTriggerSource`，修复 = 转谱器把 BGM/scratch 的 `Samples` 置空、键音走 store 的 `KeysoundSample`；用户多谱实测 ✅、转谱 19/19 + BMS 871/871 + Release 0 错。此前 2026-06-07 J6 已修长 BGM 被 store 通道偷断（floor 128）+ tap-note→store per-WAV cut；转谱键音重复的 LN 部分、dense D 卡顿、选歌预览 Track 泄漏仍后置。详见 [CHANGELOG](CHANGELOG.md) 与 [P1-J 四件套](../subline/P1-J/)）
...
- **BMS→mania 转谱音频补全（K11 转谱对象 + J6 mania-runtime 播放，已闭合）**：此前 `BMS -> mania` 转谱只搬玩家可击打对象的键音、BGM（autoplay channel `0x01`）被静默丢弃 → 纯键音 BMS 在 mania 游玩（mania ruleset + `ShowConvertedBeatmaps`）丢掉鼓/贝斯/铺底/人声等背景层。`K11`：BGM 以 sample-only `BmsConvertedBgmSampleHitObject` autoplay 发声、不进 scorable/star，LN 尾 node sample 置空对齐 BMS「长条只头发声」。`J6`：转谱 BGM/scratch/tap-note 统一走复用的 `BmsKeysoundStore`（`DrawableManiaRuleset` 反射宿主 + 缓存 + 挂载游玩树，暂停/seek 统一停、缺席安全回退），已闭合 **E（暂停停 BGM）/ 长 BGM 通道偷断（store floor 128）/ tap-note→store per-WAV cut（生产默认）/ bgm1 按键触发（BGM/scratch `Samples` 置空、键音走 `KeysoundSample`；真因 = mania 按键音效反馈 `GameplaySampleTriggerSource`，2026-06-08 用户多谱实测 ✅，见 P1-J CONSTRAINTS #11）**。仍后置：转谱键音重复的 **LN 部分**（须池化嵌套头）、tap-note 非池化 drawable 的 dense perf、**dense D 卡顿**、选歌预览 Track 泄漏。详见 [P1-J 四件套](../subline/P1-J/) 与 [P1-K CHANGELOG](../subline/P1-K/CHANGELOG.md)。
