...
| 3 | Skin V1 共同合同 | `SV1-1` 三态/precedence、semantic taxonomy、lane identity/topology/保持性 validation、config bucket presence、legacy mania primitive scalar accepted snapshot、event envelope/order、capability negotiation、六类 lane-resource candidate plan 与 internal resolution/owner 十一切完成；生产未接入 | 继续 native context/revision/wire、数组/颜色/global config、真实资源 validation/materialization/shared codec、concrete event payload/producer、真实 capability manifest/runtime 与 full layout fixtures |
...
| 子线 | 当前状态 |
| --- | --- |
| P1-A | `SV1-0` 自动/数据/实机全过；`SV1-1` 前十一个合同切片完成，未接生产链 |
...
### 2026-07-14 P1-A `SV1-0` / `SV1-1` 前十一切

用户自行确认完整皮肤实机清单正常；前九切保留 lane-resource shared/mania/BMS 12/12、6/6、29/29 与 resolution/owner 55/55、capability 91/91。第十切新增 neutral topology-preserving validator 12/12；第十一切新增 legacy mania 九个 primitive scalar 的 decoder accepted-declaration/value snapshot 18/18，连同 decoder/declaration 30/30。shared gameplay 235/235、mania relevant 120/120、BMS relevant 108/108，core skin 仍为同名既有 57/62，`osu.Desktop.slnf` Release 0 error / 20 warnings；第九切 BMS 全量 1117/1117 未在后两切重跑。scalar snapshot 不含数组/颜色/global resources/`NoteBodyStyle`，不修 legacy parser 时序，也不是完整 neutral config；native keymode、revision/wire、full layout/geometry、真实文件 validation/materialization/shared codec、concrete event payload/producer、真实 manifest/授权/runtime、scene/script ABI 与生产接线均未完成。精确矩阵与告警归因见 [P1-A STATUS](../subline/P1-A/DEVELOPMENT_STATUS.md)。
