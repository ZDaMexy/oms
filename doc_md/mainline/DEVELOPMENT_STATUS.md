...
> 最后更新：2026-06-15（**P1-L BGA Phase 5/5.1**：BMS 游玩 BGA 背景图·动画·视频链路落地，老式视频经 opt-in 外部 ffmpeg 转码播放；自动化通过、人工视觉验收待办）。当前真实状态见下「最新快照」，最新验证快照见「最近一次验证」，历史切片见 [CHANGELOG.md](CHANGELOG.md)——按本页规则「最近一次验证只保留最新一条；历史归 CHANGELOG」，抬头不再堆叠历史快照。
...
| P1-C 判定语义与反馈闭环补强 | BEATORAJA / LR2 / IIDX parity / FAST/SLOW / judge display / BMS 结果页反馈面 / visual timing-offset / EX pacemaker / 权威 GN 与调速反馈 / pre-start 1 号普通轨纯视觉流速预览 | 已阶段性收口（feedback family、tri-mode/pre-start 链、results-side consumer proof 已落地；判定 parity 第 1–2 刀已收口，剩 gameplay 反馈区分第 3 刀。明细见下「遗留问题」与 [P1-C](../subline/P1-C/)） |
