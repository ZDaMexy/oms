...
| BMS 全量测试 | **812/812** | 最近一次全量 `osu.Game.Rulesets.Bms.Tests`（2026-05-23） |
...
### 2026-05-29（P1-L Phase 2 Step A–C：BMS 专用滚动位置积分旁路，门控默认 OFF）

- **范围**：`P1-L` 落地 Phase 2 Step A–C——beatoraja 风格逐对象位置积分旁路，让 DEAD SOUL [Revive] 的「瞬移 snap / STOP 真冻结 / measure-length 任意定高」成立。**红线全守**：绕开而非改写共享核心（不动 `TimingControlPoint` 钳制、不动 `ScrollingHitObjectContainer`，注入全在 BMS 侧）；判定/计分继续走 `HitObject.StartTime` 时间链路、语义不变；门控默认 OFF、可一键回退。
- **本轮**：Step A 新增 `BmsScrollProfile`（converter `buildEventTimeline` 并行积分**原始未钳制** `D(t)`，STOP 段真冻结、132 万 BPM 段 snap）挂 `BmsBeatmap.ScrollProfile`；Step B 新增 `BmsStopMotionScrollAlgorithm : IScrollAlgorithm` + `BmsScrollingInfo`（包裹基类、Algorithm 默认逐实例跟随、可 engage/disengage），由 `BmsPlayfield.CreateChildDependencies` 重缓存注入，门控 `BmsGimmickScrollMode{Off/On/Auto}` 默认 `Off`；Step C 标定 + 设置面板「演出谱滚动」下拉 + 端到端冻结测试。
- **标定结论（实测 DEAD SOUL）**：STOP 冻结占 43.1%、snap 10000×、地雷 5645；`GetMostCommonBeatLength`=**6（BPM 10000）**证实正常链路对 132 段 squash。但默认 Normal hi-speed 模式 `timeRange` 与 `GetMostCommonBeatLength` 无关（modeScale=1）+ profile base=132 → **Normal 模式零标定即忠实**；Floating/Classic 绝对刻度标定归 Phase 4。
- **测试 / 构建**：新增 `BmsScrollProfileTest`(11)/`BmsStopMotionScrollAlgorithmTest`(5)/`BmsScrollingInfoTest`(4) + 扩 `BmsBeatmapConverterTest`（profile 冻结/退化/snap/端到端共 +7）；BMS 全套 **854/854**；`dotnet build osu.Desktop.slnf -p:Configuration=Release` 0 错误、生产代码 0 新增警告。正常链路无回归：OFF 时算法逐实例跟随基类（单测锁定）+ Player 系 gameplay TestScene（实跑 DI 重缓存）全绿。
- **门控默认 `Auto`**（用户拍板）：Step D 自动检测 `IsStopMotionGimmick` 已落地，特效/变速谱开箱即用、正常谱不命中检测零改动，`Off` 为硬回退。**已知限制 / 下一步**：DEAD SOUL 逐帧**人工视觉验收**未做（交接，归 Phase 4，验证法：设置→BMS→「特效谱滚动」实跑；用户实跑反馈观感已对路）；Floating/Classic 标定、负向/反向滚动（Phase 3）未做；极端谱地雷/对象池性能与 P1-J 协同。同日另修 Phase 1 地雷 off-by-one（最右键轨道地雷被误丢，已加 `BmsRuleset.GetLaneCount` + 回归）。
- **2026-05-29 早 / 2026-05-28 历史快照**（已归档 [CHANGELOG.md](CHANGELOG.md)）：同日 P1-L 建线 + Phase 1 地雷视觉（仿小节线、完全隔离，BMS 全套 831/831）；同日早 P1-K 解析→呈现链路全量修复；更早 K9/K10 邻接收口、K10 carousel 性能闭环、K9 三刀、K8~K4、`P1-J` 等。本状态页只保留最新一条快照。
