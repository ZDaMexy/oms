...
OMS 处于 Phase 1.x 收尾期。P1-A `SV1-0` 的自动、schema 56 数据与用户实机 gate 已全部通过；`SV1-1` 的三态/precedence、semantic slot taxonomy 与 neutral lane identity primitives 三个切片已完成但未接生产链。详见 [P1-A STATUS](../subline/P1-A/DEVELOPMENT_STATUS.md)。
...
| 3 | Skin V1 共同合同 | `SV1-1` 三态/precedence、semantic taxonomy 与 lane identity 三切完成；生产未接入 | 继续 topology aggregate/adapter 与 config/event/capability fixtures |
...
- **lane identity**：强类型 GroupId/LaneId、group/lane identity、`Neutral/Primary/Secondary` side 与 `Key/SpecialKey/Scratch` role 已落；尚无 aggregate、索引/geometry、真实 mania/BMS adapter 或 wire ABI。
...
| 子线 | 当前状态 |
| --- | --- |
| P1-A | `SV1-0` 自动/数据/实机全过；`SV1-1` 前三个合同切片完成，未接生产链 |
...
### 2026-07-14 P1-A `SV1-0` / `SV1-1` 前三切

用户自行确认完整皮肤实机清单正常；slot+identity shared focused 73/73、provider authority 6/6，mania/BMS focused 保持通过，core skin 仍为同名既有 5 项失败，`osu.Desktop.slnf` 强制 Release Rebuild 0 error / 20 warnings。identity primitives 仍不是 layout aggregate、manifest/event ABI 或生产能力；精确矩阵、告警与首跑 fixture/harness 失败归因见 [P1-A STATUS](../subline/P1-A/DEVELOPMENT_STATUS.md)。
