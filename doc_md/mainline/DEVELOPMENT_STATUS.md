...
| BMS 全量测试 | **871/871** | 最近一次全量 `osu.Game.Rulesets.Bms.Tests`（Release，2026-06-08） |
...
### 2026-06-08（P1-J：转谱-mania「按 key1 触发 bgm1 / 长音重叠 / 暂停不停」已解决，用户多谱实测 ✅）

- **真因**：mania `Column.OnPressed` 每次按键经按键音效反馈 `GameplaySampleTriggerSource` 播**本列下一个对象的 `Samples`**（自带一池非循环、不受 store 暂停管的 sound）。转谱 BGM/scratch sample-only 对象被钉在可玩列（BGM→column 0）、其 `Samples` 装着键音（bgm1）→ 按 key1 即经反馈反复触发 bgm1、重叠、绕开 store 故暂停不停。一个根因解释全部现象，也解释了为何 store / hit-object 埋点全抓不到。
- **修复**：`BmsToManiaBeatmapConverter` 把 `BmsConvertedBgmSampleHitObject` / `BmsConvertedScratchSampleHitObject` 的 `Samples` 置空（键音只放 `KeysoundSample`、经 shared `BmsKeysoundStore` 自动发声）；只动转谱器、不碰 osu 核心。回归守卫：`BmsToManiaBeatmapConverterTest` 断言 BGM/scratch `Samples` 必空、键音在 `KeysoundSample`。约束见 [P1-J CONSTRAINTS #11](../subline/P1-J/TECHNICAL_CONSTRAINTS.md)。
- **诊断方法**：隐藏发声路径（既不经 store 也不经 hit-object `PlaySamples`）用最底层 `PoolableSkinnableSample.Play()` 加调用栈探针一栈定位；本会话先后试的 orphan-on-reuse（已回退）/ LN-head（已否定）/ Track 泄漏（已回退）均与本 bug 无关、作废。
- **测试 / 构建**：`BmsToManiaBeatmapConverterTest` **19/19**、`osu.Game.Rulesets.Bms.Tests` **871/871**、`osu.Desktop.slnf` Release **0 警告 0 错误**（本次专项文档梳理已重跑核实）。**用户真实 app 多谱实测**：按 key1 不再触发 bgm1、其他原本同问题谱面一并修好。
- **更早快照**（2026-05-31 难度表全 Unrated 真根因=转谱星数与难度表共用 `RulesetData` 列互相覆盖、修复=两侧 `[JsonExtensionData]`，判读顺序见下「遗留问题 → 难度表一致性」；P1-L 滚动旁路 + 地雷、P1-J 键音链路、K9/K10 邻接与 carousel 性能等）均已归档 [CHANGELOG.md](CHANGELOG.md)。本状态页只保留最新一条快照。
