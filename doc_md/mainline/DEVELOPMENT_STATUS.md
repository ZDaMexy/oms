...
| P1-J BMS gameplay runtime 性能与音频时序治理 | shared keysound pool 时序 / dense-lane hot path / live channel resize 安全合同 / dense full autoplay replay 分流 | 进行中（`J1` / `J4` 已完成，`J2` / `J3` 首刀已落地，`J5` 自动化已闭合；full autoplay 专用 replay 分流与 keysound 预热已落地，剩余 once-per-run hitch 现场确认与人工验收；J6 转谱音频已闭合 E（暂停停 BGM）/ 长 BGM 通道偷断（store floor 128）/ tap-note→store per-WAV cut（生产默认）/ bgm1 按键触发（BGM/scratch `Samples` 置空，2026-06-08 用户多谱实测 ✅）/ tap-note 池化（2026-06-10 经接口路由 store）/ 游玩期帧抖动（2026-06-11 同样本快路径，用户实测 ✅）/ 开局 gen2 冻结（2026-06-11 prewarm 放开玩家模式）；转谱键音重复的 LN 部分、50k 极端 dense（未 profile）、选歌预览 Track 泄漏仍后置） |
