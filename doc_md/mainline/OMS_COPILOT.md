...
If OMS later adds a pre-start speed preview, keep it strictly scoped to the blocked-start window while `UI_PreStartHold` is actively held and actual gameplay has not yet started. Implement it as a pure visual preview layer on the first non-scratch lane using the current BMS note skin lookup and scroll/time authority (for example via `BmsPlayfield` / `BmsHitObjectArea` / `BmsScrollSpeedMetrics`), and let it follow the same pre-start clock / pause state as the visible playfield. Do not model it as a real `BmsHitObject` / `DrawableBmsHitObject`, do not route it through lane hit handling, and do not allow it to affect judgement, score, replay, autoplay, or keysound behaviour. Once actual gameplay starts, the preview must disappear even though the same held action continues to adjust Hi-Speed.
