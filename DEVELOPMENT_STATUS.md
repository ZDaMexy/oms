...
| BMS 单测验证 | 通过 | `2026-04-09` 最近一次全量 `dotnet test osu.Game.Rulesets.Bms.Tests.csproj --no-restore` **463/463** 通过；同日直接受影响过滤回归 **69/69** 通过 |
| 关键回归测试 | 通过 | `2026-04-09` 最近一次 BMS HID handler 定向回归（`OmsHidDeviceHandlerTest`）**14/14** 通过；`2026-04-10` 最近一次 Mania 定向回归（`OmsOwnedSkinComponentContractTest` + `TestSceneOmsBuiltInSkin`）**71/71** 通过；最近一次 results-panel contract 受影响 BMS 过滤回归 **69/69** 通过 |
