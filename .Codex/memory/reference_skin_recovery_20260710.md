...
- 恢复后 `SV1-2` 第一刀只新增无 production caller 的 authority/path lexical preflight；它不是安全 folder store、scanner、mutation token 或热重载。新地雷见 [[reference_skin_filesystem_authority_preflight]]。
...
3. G1 仍须按 managed/external、安全删改、扫描 authority、热重载四个独立切片重做；当前只完成进入第一项所需的声明/preflight 原语，immutable revision、生产选择、owner tag、mutation 与 reload 仍未完成。测试按实际改动面选择，修改 shared/mania/fallback authority 时才强制追加 core/mania gate。
