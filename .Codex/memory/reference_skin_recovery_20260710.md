...
- 恢复后 `SV1-2` 已新增两个无 production caller 的内部地基：authority/path lexical preflight 与 pure post-capture immutable capsule；二者都不是 Windows native no-follow capture、安全 folder store、scanner、mutation token 或热重载。地雷见 [[reference_skin_filesystem_authority_preflight]]、[[reference_skin_package_revision_capsule]]。
...
3. G1 仍须按 managed/external、安全删改、扫描 authority、热重载四个独立切片重做；当前只完成声明/preflight 与 pure capsule 原语，Windows native capture、生产选择、owner tag、mutation 与 reload 仍未完成。测试按实际改动面选择，修改 shared/mania/fallback authority 时才强制追加 core/mania gate。
