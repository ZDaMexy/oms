...
- 当前受管目录的`BmsLegacySkin`实例绑定exact immutable capsule，磁盘原地变化不会混入active preparation，也不会自动reload；取得新来源仍需prepared revision/new-instance切换。全consumer publication barrier与旧owner detach归`SV1-2`整包reload，不得把selection pair或逐组件A→B描述成完整热重载。2026-08-09只读产品链审计因无真实caller、BMS playfield不监听geometry更新及全renderer无detach协议而判NO-GO；稳定地雷见[[reference_skin_atomic_reload_detach]]。
