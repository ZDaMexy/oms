...
- 当前go/no-go：C1已关闭external Workspace、exact-set mutation与full ManagedCopy；C2 current external/managed/ordinary mutation和atomic reload/detach也已签发，C3/C4/C5 package+layout+material+scene/event不改变这些物理边界，当前燃尽为`5/7 closed，C6 active`。旧通用`Delete/CanModify`及thin/arbitrary-path stager继续冻结；不能把held-authority/capsule+manifest/single-v3纵切退化成普通递归copy。
