...
- [gameplay skin config presence](reference_gameplay_skin_config_presence.md) — bucket/legacy mania scalar/indexed-array/global/per-column-colour/bucket-global-resource accepted presence、semantic resource mapping、stable-lane colour mapping、per-index mask、synthetic default、legacy mania/native BMS 双侧 `ImageLookups` factory 前可变窗口与 decoder authority 地雷。
