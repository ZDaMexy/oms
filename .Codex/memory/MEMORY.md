...
- [构建与测试](reference_build_and_test.md) — CLI gate、恢复基线、targeted formatter、脚本转义与 C# Dev Kit 误判地雷。
...
- [gameplay skin lane identity/topology](reference_gameplay_skin_lane_identity.md) — 强类型 stable ID、immutable topology snapshot、neutral transition validator、internal BMS/mania projection 与 native-context 盲区。
- [gameplay skin config presence](reference_gameplay_skin_config_presence.md) — bucket/legacy mania scalar/indexed-array accepted presence、per-index mask、synthetic default、native mutation 与 decoder authority 地雷。
