...
- managed Windows fixed root handle + handle-relative/no-follow capture、schema 57 exact-owner scanner、production exact-capsule factory/选择、公共mutation/recovery foundation、directory-only rename及fixed-source staged import已实现，地雷见 [[reference_skin_windows_handle_capture]]、[[reference_skin_managed_folder_selection]]与[[reference_skin_managed_folder_mutation_foundation]]；factory与staged-import publisher仍只能消费完整成功的capsule。managed delete、external capture与atomic reload publication尚未实现，rename/import UI也仍冻结。
