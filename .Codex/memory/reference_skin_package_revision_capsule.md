...
- `SkinPackageRevisionCapsuleFactory` 是 ruleset-neutral 的 **post-capture container factory**，不是 filesystem capture service。输入只有 producer 提供的逻辑 file/directory entries、declared length 与 read callback；自身无filesystem dependency，现已有managed native internal producer，但仍无`SkinManager`/production managed folder factory/选择消费方。
...
- managed Windows fixed root handle + handle-relative/no-follow capture已实现，地雷见 [[reference_skin_windows_handle_capture]]；production factory仍必须只消费它完整成功后的capsule。external capture、选择、scanner、mutation与atomic publication尚未实现。
...
- production managed folder factory 后续必须走 exact-capsule 构造路径，不能让 live `RealmBackedResourceStore` 排在 capsule 前面。普通 `.osk` 与 SkinEditor 当前依赖 Realm live store/refresh，不在本刀全局冻结；其原子更新另走 prepared revision/new-instance 协议。
...
- 当前完成度、测试数字与production managed folder factory下一门只看P1-A STATUS/PLAN；不得把本合同写成G1、folder skin或Skin V1产品能力。
