...
- managed Windows fixed root handle + handle-relative/no-follow capture、schema 57 exact-owner scanner、production exact-capsule factory/选择、公共mutation/recovery foundation、directory-only rename及fixed-source staged import已实现，地雷见 [[reference_skin_windows_handle_capture]]、[[reference_skin_managed_folder_selection]]与[[reference_skin_managed_folder_mutation_foundation]]；factory与staged-import publisher仍只能消费完整成功的capsule。settings managed delete现已通过专用held authority/journal/recovery接通，但不能从capsule类推通用delete capability；旧通用delete、external capture、atomic reload publication及rename/import UI仍冻结。
