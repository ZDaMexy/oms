...
- active capsule的单一owner必须先detach全部participant/work/operation lease再dispose。directory-only rename不销毁当前active capsule；fixed-source staged import也不自动选择或替换active capsule，且不会取消无关pending selection。C2已签发统一barrier与retire queue，C3/C4/C5已把layout/material/scene作为同一package+layout+material+scene publication接入；具体见[[reference_skin_atomic_reload_detach]]，C6新增owner继续接入。
