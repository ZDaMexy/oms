...
- active capsule 的单一 owner 必须先 detach 全部 participant/work/operation lease 再 dispose。directory-only rename不销毁当前active capsule；fixed-source staged import也不自动选择或替换active capsule，且不会取消无关pending selection。C2已签发统一barrier与retire queue；具体见[[reference_skin_atomic_reload_detach]]，C3～C6新增owner继续接入。
