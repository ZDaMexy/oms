...
- `SkinFilesystemStorageResolver` 只做 schema 56 storage declaration 与检查当时的 lexical/reparse preflight；managed成功结果现会签发internal capture request，但仍无`SkinManager`/production managed folder factory/选择消费方。
...
- preflight → managed Windows native no-follow producer → pure capsule内部链已实现，但仍无`SkinManager`/production managed folder factory/选择消费方。external的resolved-identity/capture仍未实现；其`NativeStorage`以后只能作为只读source adapter，parser/decoder只能消费完整成功后交付的OMS自有capsule，不能持续直读可变live folder。
- scanner 开工前仍需 nullable opaque persistent owner token；null/unknown、`.osk`、另一 root/authority与不完整扫描中未见的记录都不能自动清理。
- production managed folder factory 必须精确允许 `InstantiationInfo`，不能走 `SkinInfo.CreateInstance()` 的历史 `TrianglesSkin` fallback。整包 reload 还需要 generation/current-selection/revision gate、全 consumer publication barrier和旧 owner安全退役。

## 验证与工作流

- 第一刀 fixture 同时锁 managed/external bytes、mtime 与 `SkinInfo` 不变，避免“只比较目录项名称”产生零写入假阳性。
- capture已有真实Windows package/nested junction、hardlink、busy writer与反向share/final-inventory gate；mutation service仍需独立真实junction/rename/delete门，不能用只读capture测试关闭完整G1安全门。
