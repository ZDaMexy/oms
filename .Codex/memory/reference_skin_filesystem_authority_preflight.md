...
- `SkinFilesystemStorageResolver` 只做 schema 56 storage declaration 与检查当时的 lexical/reparse preflight；managed成功结果会签发internal capture request，后续production factory/guarded selection现可消费完整native capture capsule，但resolver结果本身仍不是选择授权。
