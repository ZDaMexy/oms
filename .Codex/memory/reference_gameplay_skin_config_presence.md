...
description: C4 后仍有效的 legacy decoder accepted-token/presence 地雷，以及已删 configuration snapshot/factory 的历史兼容语义
metadata:
  node_type: memory
  type: reference
---
...
> **C4召回边界：**当前仍可用的中性载体只有bucket declaration与已接production的lane-resource snapshot/candidate；`GameplaySkinLaneColourSnapshot`、legacy mania primitive/array/global-colour/global-resource/`NoteBodyStyle` snapshot factory，以及BMS bucket colour/geometry/declaration snapshot factory均已删除。下文相应“snapshot/factory”小节只保存retired API曾锁定的legacy parser accepted-token语义，不能据此恢复类型、建立第二套public catalog或核算产品进度。当前public authoring/runtime一律以shared document、28项catalog、唯一resolver及exact material publication为准。
...
## 历史：legacy mania primitive scalar accepted snapshot（factory已删除）
...
- legacy adapter仍须保护`flushPendingLines()`异常前不清空坏行、malformed`Keys`沿用旧current config、duplicate`Keys`后续字段写入discarded config等兼容事实；C4 shared codec已经为public section另立严格malformed/duplicate合同，不得反向“修正”legacy行为或把它升级成public V1语义。

## 历史：legacy mania NoteBodyStyle accepted snapshot（factory已删除）
...
- 真实validation/materialization仍受package authority边界约束；C4已把BMS Note/LN与mania Note/Hold/KeyVisual接入shared codec/resolved material和三源原子publication。其它C5资源、scene/script及最终G1整包门是否完成只看P1-A STATUS，不得从本snapshot推断。

## 历史：native BMS exact colour accepted snapshot（factory已删除）
...
- 这是legacy accepted provenance，不自行提供validation/solver/materializer。`LongNoteBodyWidth`仍只接受finite且`0 < width <= 1`并按typed reason回到`0.5775`；C3/C4现已确保body素材帧、resolved width、layout与material来自同一exact publication，renderer不得重读可变aggregate。其余legacy字段是否进入public/runtime只看catalog与capability，不能由sidecar存在推断。

## 历史：legacy mania indexed-array accepted snapshot（factory已删除）
...
## 历史：legacy mania known global colour accepted snapshot（factory已删除）
...
## 历史：legacy mania per-column colour accepted snapshot（factory已删除）
...
- 该历史snapshot只冻结accepted provenance与中性映射，不是public manifest/wire ABI。C4已删除被exact resolved material取代的lane-colour snapshot factory；保留compatibility映射必须进入唯一production resolver或只作为明确fixture seam，不能继续用snapshot数量核算产品能力。

## 历史：legacy mania bucket-global resource accepted snapshot（factory已删除）
