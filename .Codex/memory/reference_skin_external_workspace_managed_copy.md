...
- external Workspace、ManagedCopy与noncurrent external Unregister已随C1关闭；C2 same-ID reload、current external unregister与全consumer publication/detach/retire也已签发，C3/C4又把layout/material纳入不可分割publication，C5再接入scene/event与全部适用slot host；当前燃尽为`5/7 closed，C6 active`。ordinary Realm `.osk`使用Delete而非Unregister；watcher始终不属于该能力。
