...
- cleanup必须在typed失败、取消、意外异常和某个handle `Dispose()`抛出时继续释放其余handle，并清理provisional capsule。managed一次性capture成功结果不得携带live handle、stream或deferred filesystem callback；external held session则有意把proof/capsule/manifest与handle生命周期保持到调用方final复验，随后必须在取消、stale、shutdown或提交后完整释放。
