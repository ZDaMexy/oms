...
- 计分与结果：EX-SCORE、CLEAR LAMP、DJ LEVEL；结果页主评价与缩略徽章已按 DJ LEVEL 显示，主分数区显式使用 EX-SCORE 语义；results 侧 final gauge / gauge history 现会复用运行时 long-note mode 与完整 beatmap-mod 链重建，避免 `HCN` body-tick fail 或 `A-SCR` / `A-NOT` assist 场景与 gameplay 脱节
- 长条模式相关计分已区分 CN / HCN
...
- 难度表：本地/在线来源管理、缓存、MD5 匹配、表分组、Song Select 音符分布图
- 谱面元数据：已解析 `#SUBTITLE`、`#SUBARTIST`、`#COMMENT`、`#PLAYLEVEL`、`#DIFFICULTY`，Song Select 可显示副标题、谱师、内部标级与表标签
- BMS 选歌 BPM：Song Select 左上 BPM 当前已按 imported chart 的真实 timing data 显示，不再对 BMS 谱面回退默认 `60`；BPM 分组与排序继续按 persisted `BeatmapInfo.BPM` 工作
- BMS 运行时调速：进入 BMS gameplay 后前 5 秒可按住“阻止谱面开始/ingame start”阻塞开谱并调节 Hi-Speed / `Sudden` / `Hidden` / `Lift`；若这段 delay 已在按住期间耗尽，则松开后会重新给满一段 fresh delay，而不是立刻开谱；正式游玩中按住同一键也可继续调节，且居中 `BMS speed` 浮窗会持续显示
- BMS pre-start 流速预览：第一版已实现；仅在 pre-start 期间按住“阻止谱面开始/ingame start”时，第一非 scratch 普通轨会显示按当前流速下落的纯视觉假音符；不参与判定 / 成绩 / 键音链，正式 gameplay 中即使继续按住同一键调速也不会再出现
- osu!mania 滚动速度设置：`Settings -> 游戏模式 -> osu!mania -> 滚动速度` 当前已通过悬浮提示明确为“标准车道几何下的参考下落时间”；不同皮肤会因车道尺寸、判定线位置与缩放改变体感，因此更换皮肤后需要按当前皮肤重新校准，且 mania 与 BMS 的下落时间不可互相参考
- 输入：键盘组合键、XInput、MouseAxis、Raw Input，以及 Windows DirectInput HID 路径；桌面端 Settings -> 输入 当前已安全隐藏上游通用的数位板 / 触屏点击 / 鼠标分区，保留 OMS 相关键位与 supplemental binding 表面，但不移除对应运行时输入链
- 发行与版本：`build-release.ps1` 当前输出 `release-repo/oms_YYYYMMDD(.zip)`；single-file 发行链现会强制完整自解压，避免 fresh extract 冷启动出现“先创建 `data/` 后无窗退出”；设置页 footer 已按 `oms + 构建日期` 显示，覆盖更新时需保留 `portable.ini`、便携模式下的 `data/`，以及任何自定义数据根使用的 `storage.ini`
- 存储：Release 默认 `%APPDATA%/oms/`，Debug 使用 `%APPDATA%/oms-development/`；BMS 导入落在数据根下 `chartbms/`，mania 导入落在 `chartmania/`，支持 `storage.ini` 迁移到自定义数据根；Settings -> Maintenance 现已拆为“外部谱库”和“内部谱库”两层：外部侧提供“扫描外部谱库（重建）/（增量）”，内部侧提供“扫描内部谱库（重建）/（增量）”；其中增量模式只补导当前尚未以同一 `FilesystemStoragePath` 处于 active 状态的目录。`Settings -> 常规 -> 安装位置 -> 更改数据目录位置` 只切换/迁移运行时数据根，不移动程序文件；空目录直接迁入，非空非数据目录会改用其下 `oms/`，已是可用数据目录则只在重启后切换
- 首次启动设置：当前为欢迎、UI 缩放、获取谱面、导入、难度表设置、按键绑定六步流程；导入页直接复用外部谱库设置，难度表页可按分组一键导入 zris 预设 URL，失败时会显示中文原因摘要，最后一步可直接配置全局、mania 与 BMS 键位
