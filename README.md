...
- 输入：键盘组合键、XInput、MouseAxis、Raw Input；HID 代码路径已接通，但 Windows 上因 HidSharp `RegisterClass failed` 初始化崩溃风险默认禁用，设置页会提示当前处于防崩溃降级，可用 `OMS_ENABLE_HIDSHARP=1` 显式开启诊断后端
- 离线模式：默认在线 endpoint 为空，在线入口与 Discord Rich Presence 受 `OnlineFeaturesEnabled` 守卫
- 最近验证：mania 定向回归 `50/50` 通过，BMS 过滤回归 `69/69` 通过，BMS HID handler 定向回归 `11/11` 通过，完整 `osu.Game.Rulesets.Bms.Tests` 为 `446/446` 通过，Debug/Release 构建通过，Release 启动 smoke 已确认不再因 HidSharp 闪退
