...
- BMS 运行时调速：进入 BMS gameplay 后前 5 秒可按住“阻止谱面开始/ingame start”阻塞开谱并调节 Hi-Speed / `Sudden` / `Hidden` / `Lift`；正式游玩中按住同一键也可继续调节，且居中 `BMS speed` 浮窗会持续显示
- BMS pre-start 流速预览：当前尚未实现；已确认应作为仅限 pre-start、固定落在 1 号普通轨的纯视觉 preview layer 落地，不进入判定 / 成绩 / 键音链
