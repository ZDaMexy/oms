...
- 存储：Release 默认使用 `%APPDATA%/oms/`、Debug 使用 `%APPDATA%/oms-development/`；BMS 导入落在数据根下 `songs/`，也支持通过 `storage.ini` 迁移到单一自定义数据根
- mania 皮肤迁移：`WidthForNoteHeightScale` 已收口到 `OmsManiaLayoutPreset`，`OmsNotePiece` 现按列读取 stage-local note-height；mixed-stage 不再把第二 stage 的 note 高度落回第一 stage / total-columns fallback
- 离线模式：默认在线 endpoint 为空，在线入口与 Discord Rich Presence 受 `OnlineFeaturesEnabled` 守卫
- 最近验证：mania 定向回归 `54/54` 通过，BMS 过滤回归 `69/69` 通过，BMS HID handler 定向回归 `14/14` 通过，完整 `osu.Game.Rulesets.Bms.Tests` 为 `463/463` 通过，Debug/Release 构建通过，Release 启动 smoke 已确认不再因 HidSharp 闪退
