...
- 存储：Release 默认 `%APPDATA%/oms/`，Debug 使用 `%APPDATA%/oms-development/`；BMS 导入落在数据根下 `chartbms/`，mania 导入落在 `chartmania/`，支持 `storage.ini` 迁移到自定义数据根
- 皮肤：OMS 内置皮肤系统正在建设中（Phase 1.1）；BMS 默认层已收口，mania 侧 OMS-owned 组件正在推进
- 离线模式：默认在线 endpoint 为空，在线入口受 `OnlineFeaturesEnabled` 守卫
- 最近验证：BMS 全量 476/476，mania 皮肤回归 81/81，BMS 皮肤 fallback 75/75，scratch bridge 43/43，构建通过
